  ...
  try {
    ...
    if (action === "poll") {
      ...
      if (provider === "kling" && klingKey) {
        // Kling status endpoints differ by mode; try both text2video and image2video status paths.
        const paths = [
        ...
        for (const url of paths) {
          try {
            ...
            const status = (d.task_status || d.status || "").toString().toLowerCase();
            // Find the video URL wherever Kling nested it.
            let vurl = "";
            const works = (d.task_result && d.task_result.videos) || d.videos || (d.works) || [];
            if (Array.isArray(works) && works.length) vurl = works[0].url || works[0].video_url || "";
            if (!vurl) vurl = d.video_url || "";
            ...
            // else still processing - fall through to try the other path or report pending
