...
// Every paid route had one, this did not. Nothing stopped one address opening a thousand Stripe sessions.
const RL = new Map();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Origin is a header the caller controls. Left as-is, someone sets Origin: https://evil.com
    // and Stripe sends the buyer there after paying - an open redirect off the back of a real
    // payment, which is exactly the shape a convincing phishing page needs.
    const ALLOWED_ORIGINS = ["https://thespine.cloud", "https://www.thespine.cloud"];
    const rawOrigin = String(req.headers.origin || "");
    const origin = ALLOWED_ORIGINS.includes(rawOrigin)
      ? rawOrigin
      : (/^https:\/\/[a-z0-9-]+\.vercel\.app$/.test(rawOrigin) ? rawOrigin : "https://thespine.cloud");
