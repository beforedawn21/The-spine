...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    const cdns=is3d
      ? ["/three.min.js","https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.min.js","https://cdnjs.cloudflare.com/ajax/libs/three.js/0.160.0/three.min.js","https://unpkg.com/three@0.160.0/build/three.min.js"]
    ...
    // Loader: tries each CDN in turn (onerror -> next). When the engine global appears, runs the game.
    // If all CDNs fail within the timeout, shows a clear message instead of a forever-black screen.
    const loader=
      ...
      +'function __runGame(){try{'+inlineJs+'\n}catch(e){__fail("Game code error: "+e.message);}}'
      +'function __loadNext(){ if(__i>=__cdns.length){ __fail("Could not load the game engine ('+engineName+'). Your network or browser may be blocking it - try again, or open the game on a different connection."); return; }'
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      +'<style>html,body{margin:0;padding:0;overflow:hidden;background:#000;width:100%;height:100%;}canvas{display:block;margin:0 auto;}</style>'
      +'</head><body>'+bodyInner
      +'<scr'+'ipt>'+loader+'</scr'+'ipt>'
      +'</body></html>';
  }
  function previewWeb(){
    let doc;
