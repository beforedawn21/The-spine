...
async function callClaudeChat(system,messages,model){
  ...
  const hasImg=Array.isArray(messages)&&messages.some(m=>Array.isArray(m.content)&&m.content.some(b=>b&&(b.type==="image"||b.type==="document")));
  const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),hasImg?60000:28000);
...
export default function App(){
  ...
  async function pullMusic(prompt){
    try{
      ...
      // Poll up to ~90s (Suno usually finishes in 30-60s). Each poll is its own quick request.
      for(let i=0;i<30;i++){
        await new Promise(r=>setTimeout(r,3000));
        let poll;
        try{
          const pr=await fetch("/api/music",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"poll",taskId})});
          poll=await pr.json();
        }catch(e){ continue; }
      ...
      return {error:"The music engine is taking longer than expected. Try again in a moment."};
  ...
  async function pullVoice(text,voiceId){
    try{
      const res=await fetch("/api/dub",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:String(text||"").trim(),voiceId:voiceId||undefined})});
      const data=await res.json();
  ...
  async function pullSfx(prompt){
    try{
      const res=await fetch("/api/audiofx",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"sfx",prompt:String(prompt||"").trim()})});
      const data=await res.json();
