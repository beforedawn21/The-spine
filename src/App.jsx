...
function extractWorldJs(raw){
  ...
  s=s.trim();
  // 8. LAST RESORT SALVAGE: if it still will not compile, peel stray lines off the
  //    top and then the bottom one at a time. A single leaked CSS declaration or a
  //    dangling word is usually all that stands between this and a working world.
  const compiles=(t)=>{ try{ new Function("SpineWorld","THREE",t); return true; }catch(e){ return false; } };
  if(s&&!compiles(s)){
    const L=s.split("\n");
    let fixed=null;
    for(let cut=1;cut<=8&&cut<L.length;cut++){
      const t=L.slice(cut).join("\n").trim();
      if(t&&compiles(t)){ fixed=t; break; }
    }
    if(!fixed){
      for(let cut=1;cut<=8&&cut<L.length;cut++){
        const t=L.slice(0,L.length-cut).join("\n").trim();
        if(t&&compiles(t)){ fixed=t; break; }
      }
    }
    if(!fixed){
      for(let top=1;top<=4&&top<L.length;top++){
        for(let bot=1;bot<=4&&top+bot<L.length;bot++){
          const t=L.slice(top,L.length-bot).join("\n").trim();
          if(t&&compiles(t)){ fixed=t; break; }
        }
        if(fixed) break;
      }
