...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    const __doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      +'try{var __iOS=/iPad|iPhone|iPod/.test(navigator.userAgent)||(navigator.platform==="MacIntel"&&navigator.maxTouchPoints>1);if(__iOS&&window.Phaser&&Phaser.Game&&typeof Proxy!=="undefined"){var __OG=Phaser.Game;Phaser.Game=new Proxy(__OG,{construct:function(t,a){(a[0]||(a[0]={})).type=Phaser.CANVAS;return Reflect.construct(t,a);}});}}catch(e){}'
      ...
      +'setTimeout(function(){try{if(document.querySelector("canvas"))return;'
      +'var __host=document.getElementById("__gamebody")||document.body;'
      // Create a real canvas element ourselves and attach it NOW - this guarantees a canvas exists in
      // the DOM regardless of whether the engine can boot. We then try to hand it to the engine.
      +'var __cv=document.createElement("canvas");__cv.width=800;__cv.height=600;__cv.style.width="100%";__cv.style.maxWidth="800px";__cv.style.display="block";__cv.style.margin="0 auto";__host.appendChild(__cv);'
      +(engineName==="Phaser"
        ? 'try{if(typeof Phaser!=="undefined"){var __sc={};if(typeof window.preload==="function")__sc.preload=window.preload;if(typeof window.create==="function")__sc.create=window.create;if(typeof window.update==="function")__sc.update=window.update;if(!__sc.create)__sc.create=function(){this.add.rectangle(400,300,780,580,0x101418);this.add.text(400,300,"Tap to play",{fontSize:"30px",color:"#fff"}).setOrigin(0.5);};__log("forcing canvas (Phaser)","#8cf");window.game=new Phaser.Game({type:Phaser.CANVAS,canvas:__cv,width:800,height:600,backgroundColor:"#101418",scene:__sc});}}catch(e){__log("engine skipped, drawing to raw canvas","#8cf");var __x=__cv.getContext("2d");if(__x){__x.fillStyle="#101418";__x.fillRect(0,0,800,600);__x.fillStyle="#fff";__x.font="30px sans-serif";__x.textAlign="center";__x.fillText("Tap to play",400,300);}}'
        : 'try{if(typeof THREE!=="undefined"){__log("forcing canvas (Three)","#8cf");var __r=new THREE.WebGLRenderer({canvas:__cv,antialias:true});__r.setSize(800,600,false);var __scn=(window.scene&&window.scene.isScene)?window.scene:new THREE.Scene();var __cam=window.camera||new THREE.PerspectiveCamera(70,800/600,0.1,1000);if(!window.camera)__cam.position.z=5;__scn.add(new THREE.AmbientLight(0xffffff,0.8));var __l=new THREE.DirectionalLight(0xffffff,1);__l.position.set(2,3,4);__scn.add(__l);if(!window.scene){var __cube=new THREE.Mesh(new THREE.BoxGeometry(1,1,1),new THREE.MeshStandardMaterial({color:0x5b8cff}));__scn.add(__cube);window.__cube=__cube;}(function __loop(){requestAnimationFrame(__loop);if(window.__cube){window.__cube.rotation.x+=0.01;window.__cube.rotation.y+=0.01;}__r.render(__scn,__cam);})();}}catch(e){__log("engine skipped, drawing to raw canvas","#8cf");var __x=__cv.getContext("2d");if(__x){__x.fillStyle="#101418";__x.fillRect(0,0,800,600);__x.fillStyle="#fff";__x.font="30px sans-serif";__x.textAlign="center";__x.fillText("Tap to play",400,300);}}'
        )
      +'}catch(e){}},1400);'
