...
TONE & MANNER: You're always truthful first - never trade accuracy for charm. But you are a genuinely JOYFUL presence, and that joy shows up in EVERY answer, not just now and then. You delight in the question, in knowing things, in helping - a brilliant, warm friend who lights up when someone asks you something. Bring energy and a spark of personality to even simple, factual, or data answers: a warm opener, a bit of character, a light touch of wit. You react to what the person actually said - acknowledge a clever question, a funny phrasing, a good idea, a hard day. You engage back with a brief, genuine follow-up when it fits. Never robotic, never flat, never a dry data-dump - even a stock price or a weather fact gets delivered with a little life. The ONE exception: if the topic is serious, sad, painful, or high-stakes, drop the levity completely and just be warm, present, and steady - joy means reading the room, not forcing cheer. Lead with the answer. Short paragraphs. Manners always: please, thank you, no condescension.
...
.topnav{display:flex;align-items:center;justify-content:space-between;padding:22px 0 20px;padding-top:max(22px,calc(env(safe-area-inset-top) + 8px));border-bottom:1px solid var(--stone);margin-bottom:34px;gap:14px;flex-wrap:nowrap;}
.topnav-actions{position:relative;display:flex;align-items:center;flex:none;}
.chats-btn{position:relative;display:flex;align-items:center;gap:6px;height:34px;padding:0 12px;border-radius:10px;border:1px solid var(--stone);background:var(--panel);color:var(--ink2);cursor:pointer;font:600 12px 'Inter';white-space:nowrap;}
...
.navwrap{display:flex;flex-direction:column;align-items:center;}
...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    const __doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      +'<style>html,body{margin:0;padding:0;width:100%;height:100%;min-height:460px;overflow:hidden;background:#000;}#__gamebody{width:100%;height:100%;min-height:460px;}canvas{display:block;margin:0 auto;}#__status{position:fixed;bottom:0;left:0;right:0;z-index:99999;background:rgba(0,0,0,.78);color:#0f0;font:11px/1.35 monospace;padding:5px 9px;pointer-events:none;white-space:pre-line;}</style>'
      +'</head><body>'
      ...
      +'if(__sized&&__drew){__log("game started OK","#0f0");try{parent.postMessage({__spineGame:true,ok:true},"*");}catch(e){}setTimeout(function(){if(__s)__s.style.display="none";},1600);return;}'
      ...
      <div className="shell">
        <nav className="topnav" aria-label="Main navigation">
          ...
          <div className="topnav-actions">
            <button className="chats-btn" onClick={()=>{if(account){refreshConvs();}setConvOpen(v=>!v);}} aria-label="Saved conversations" title="Saved conversations">
              ...
              <span className="cb-label">Chats</span>
