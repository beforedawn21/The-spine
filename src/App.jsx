...
export default function App(){
  ...
  async function generateWalkthrough(topic){
    ...
    try{
      ...
      let txt=(res&&res.text||"").trim().replace(/^```(json)?/i,"").replace(/```$/,"").trim();
  ...
  async function doDeepResearch(topic){
    ...
    try{
      ...
      let queries=[]; try{ queries=JSON.parse((planRes.text||"").replace(/^```(json)?/i,"").replace(/```$/,"").trim()); }catch(e){ queries=[q]; }
  ...
  async function doAgent(task){
    ...
    try{
      ...
      let plan; try{ plan=JSON.parse((planRes.text||"").replace(/^```(json)?/i,"").replace(/```$/,"").trim()); }catch(e){ plan={steps:[{do:q,search:q}]}; }
