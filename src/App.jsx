...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      +'<style>html,body{margin:0;padding:0;width:100%;height:100%;min-height:460px;overflow:hidden;background:#000;}canvas{display:block;margin:0 auto;}</style>'
      +'</head><body>'+bodyInner
      +'<scr'+'ipt>'
      +'(function(){'
      // Guard: many AI games read window.innerWidth/innerHeight to size the canvas. Inside an iframe
      // these can be 0 at script time, producing a 0x0 (invisible/black) canvas. Force real values.
      ...
      +'function __fail(msg){document.body.innerHTML="<div style=\\"color:#fff;font-family:sans-serif;padding:24px;line-height:1.5\\">"+msg+"</div>";}'
      +'function __runGame(){try{'+inlineJs+'\n}catch(e){__fail("Game error: "+e.message);}}'
      +'function __loadNext(){ if(__i>=__cdns.length){ __fail("Could not load the game engine ('+engineName+'). Try again or use a different connection."); return; }'
      +'var s=document.createElement("script"); s.src=__cdns[__i++]; s.onload=function(){ if(typeof '+engineName+'!=="undefined"){__runGame();} else {__loadNext();} }; s.onerror=function(){__loadNext();}; document.head.appendChild(s); }'
      +'__loadNext();'
      +'})();'
      +'</scr'+'ipt>'
      +'</body></html>';
