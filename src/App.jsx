...
export default function App(){
  ...
  const ADMIN_USER="BEFOREDAWN", ADMIN_PASS="beforedawn"; // exact case triggers the Brain Stem
  const [adminOpen,setAdminOpen]=useState(false);
  const [adminAuthed,setAdminAuthed]=useState(false);
  useEffect(()=>{ if(adminAuthed){ (async()=>{ const s=await loadAdminStats(); if(s)setAdminStats(s); })(); } },[adminAuthed]);
  ...
  async function doCore(){
    ...
    // Rate limit (backstop): max questions per rolling hour
    const now=Date.now();
    rateRef.current=rateRef.current.filter(t=>now-t<3600000);
    if(rateRef.current.length>=(settings.rateLimitPerHour||RATE_LIMIT_PER_HOUR)){
    ...
    for(let i=0;i<CORE_STAGES.length;i++){
      ...
      if(CORE_STAGES[i].key==="reason"){
        if(ensembleMode){
          ...
          const outs=await Promise.all(labels.map(()=>callClaudeChat(CORE_SYS,history,chosenModel)));
          setEnsembleOuts(outs.map((o,idx)=>({label:labels[idx],text:o.text,live:o.live})));
          answer=outs[0].text;live=outs[0].live;
        }else{const res=await callClaudeChat(CORE_SYS,history,chosenModel);answer=res.text;live=res.live;}
  ...
  function siVerify(){
    ...
    (async()=>{
      const isAdmin=siName.trim()===ADMIN_USER&&siPass===ADMIN_PASS;
  ...
  function siSignIn(){
    ...
    if(siName.trim()===ADMIN_USER&&siPass===ADMIN_PASS){
          ...
          <div className="g2">
            ...
            <div className="panel">
              ...
              <div className="core-input-row">
                <button className="attach-btn" onClick={()=>fileInRef.current&&fileInRef.current.click()} disabled={coreBusy} aria-label="Attach a file">
                  <svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m21.44 11.05-9.19 9.19a6 6 0 0 1-8.49-8.49l8.57-8.57A4 4 0 1 1 18 8.84l-8.59 8.57a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
                  Attach
                </button>
                {coreBusy?(
