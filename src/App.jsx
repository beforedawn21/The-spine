...
export default function App(){
  ...
  async function runPipeline(){
    ...
    if(p.key==="game"){
      if(gamesRemaining()<=0){ setPipeOut({text:"You've used your 2 free games. Sign up (it's free) to keep building - and to post your games to the marketplace.",live:false,name:p.name,key:p.key,needSignup:true}); return; }
    ...
    if(p.key==="game"&&res.live){ if(!account) setGamesUsed(n=>n+1); setGameClarify(null); setGameClarifyText(""); }
  }
  function previewWeb(){
    const code=(()=>{const m=(pipeOut?.text||"").match(/```[a-zA-Z]*\n([\s\S]*?)```/);return m?m[1]:pipeOut?.text||"";})();
    // For games, inject the engine library (Phaser for 2D, Three.js for 3D) so it actually runs in the iframe.
    let doc=code;
    if(pipeOut&&pipeOut.key==="game"){
      const lib=pipeOut.gameMode==="3d"
        ? '<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/0.160.0/three.min.js"></script>'
        : '<script src="https://cdnjs.cloudflare.com/ajax/libs/phaser/3.80.1/phaser.min.js"></script>';
      // If the AI returned a full HTML doc, inject the lib into the head; otherwise wrap the code.
      if(/<html[\s>]/i.test(code)){
        doc=code.replace(/<head[^>]*>/i,m=>m+"\n"+lib);
        if(!/<head/i.test(code)) doc=code.replace(/<body[^>]*>/i,m=>lib+"\n"+m);
      }else{
        doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+lib+'<style>html,body{margin:0;padding:0;overflow:hidden;background:#000;}canvas{display:block;}</style></head><body>'+code+'</body></html>';
      }
    }
  ...
  // Games: admins unlimited; signed-in accounts unlimited (gated elsewhere by credits when enabled);
  // visitors with no account get 2 free, then must sign up.
  function gamesRemaining(){
    if(account&&account.admin) return Infinity;
    if(account) return Infinity;
    return Math.max(0, 2-gamesUsed);
  }
  ...
  function playMarketGame(t){
    const code=(()=>{const m=(t.content||"").match(/```[a-zA-Z]*\n([\s\S]*?)```/);return m?m[1]:t.content||"";})();
    const lib=(t.gameMode==="3d")
      ? '<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/0.160.0/three.min.js"></script>'
      : '<script src="https://cdnjs.cloudflare.com/ajax/libs/phaser/3.80.1/phaser.min.js"></script>';
    let doc;
    if(/<html[\s>]/i.test(code)){
      doc=/<head/i.test(code)?code.replace(/<head[^>]*>/i,m=>m+"\n"+lib):code.replace(/<body[^>]*>/i,m=>lib+"\n"+m);
    }else{
      doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+lib+'<style>html,body{margin:0;padding:0;overflow:hidden;background:#000;}canvas{display:block;}</style></head><body>'+code+'</body></html>';
    }
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {p.key==="game"&&(<>
                  ...
                  {gamesRemaining()!==Infinity&&<p className="sub" style={{marginTop:6}}>{gamesRemaining()} free {gamesRemaining()===1?"game":"games"} left. Sign up (free) for unlimited + to post to the marketplace.</p>}
                </>)}
                <div className="code-bar" style={{marginTop:12}}>
