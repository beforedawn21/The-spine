...
async function callClaude(system,user){
  ...
  try{
    const res=await fetch("/api/chat",{
      ...
      signal:ctrl.signal,
      body:JSON.stringify({system:(system||"")+STYLE_RULE,user:user||"(no input)"}),
    });
...
async function callClaudeChat(system,messages){
  ...
  try{
    const res=await fetch("/api/chat",{
      ...
      headers:{"Content-Type":"application/json"},
      signal:ctrl.signal,
      body:JSON.stringify({system:(system||"")+STYLE_RULE,messages}),
    });
    clearTimeout(timer);
    let data;
    try{ data=await res.json(); }catch(parseErr){
      return{text:"(Debug) Server responded with status "+res.status+" but the body wasn't valid JSON. Likely a server-side crash.",live:false};
    }
    if(data&&data.error){
      return{text:"(Debug) API error — "+(data.error.type||"unknown")+": "+(data.error.message||JSON.stringify(data.error)),live:false};
    }
    const text=(data.content||[]).map(b=>b.type==="text"?b.text:"").join("").trim();
    if(!text){
      return{text:"(Debug) Got HTTP "+res.status+" but no usable text. Raw response: "+JSON.stringify(data).slice(0,500),live:false};
    }
    return{text,live:true};
  }catch(e){clearTimeout(timer);
    return{text:"(Debug) Request itself failed before reaching the server — "+(e&&e.message?e.message:String(e)),live:false};
  }
}
...
export default function App(){
    ...
    (async()=>{
      ...
      if(savedId){
        ...
        if(alive&&acct){
          ...
          const cv=await loadConversations(acct.id); if(alive&&cv)setSavedConvs(cv);
        }
      }
    })();
    return ()=>{alive=false;};
  ...
  async function doCore(){
    ...
    const history=baseThread.slice(-12).map(m=>({role:m.role,content:m.text}));
    ...
    for(let i=0;i<CORE_STAGES.length;i++){
      ...
      if(CORE_STAGES[i].key==="reason"){
        if(ensembleMode){
          ...
          const outs=await Promise.all(labels.map(()=>callClaudeChat(CORE_SYS,history)));
          setEnsembleOuts(outs.map((o,idx)=>({label:labels[idx],text:o.text,live:o.live})));
          answer=outs[0].text;live=outs[0].live;
        }else{const res=await callClaudeChat(CORE_SYS,history);answer=res.text;live=res.live;}
  ...
  function siVerify(){
    ...
    (async()=>{
      const isAdmin=siName.trim().toLowerCase()===ADMIN_USER;
              ...
              <div className="thread">
                {thread.length===0&&!coreBusy&&(
                  <div className="greet">
                    ...
                    <div className="greet-sub">Ask me anything. Your request travels up the spinal cord — gathering sources, routing to the best model, reasoning, and verifying — then fires an answer back, every step recorded.</div>
                  </div>
                )}
                {thread.map((m,idx)=>(
                  m.role==="user"?(
                    ...
                    <div key={idx} className="msg msg-ai">
                      ...
                      <div className="msg-ai-foot">
                        {m.live?<span className="run-live inline"><span className="dot"/>Reasoned live, verified</span>:<span className="run-live off inline"><span className="dot"/>Preview — live model unavailable</span>}
