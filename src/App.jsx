...
export default function App(){
  ...
  async function doCore(){
    ...
    // TEMP DIAGNOSTIC: also fire if the question looks like a niche factual lookup, so we can confirm Tavily works.
    const looksFactual=/\b(frame data|stats|price|score|release date|who is|what year|when did|latest|current|how many)\b/i.test(q);
    if(live&&(answerSignalsGap(answer)||looksFactual)&&settings.killSearch!==true){
      setThread(t=>[...t,{role:"assistant",text:"",lookupPending:true,at:new Date()}]);
      const result=await tryLookup(q);
      if(result&&result.answer){
        setThread(t=>t.map((m,i)=>i===t.length-1?{role:"assistant",text:"Let me check that for you — here's what I found:\n\n"+result.answer,live:true,at:new Date()}:m));
      }else{
        const dbg=result&&result.debug?("(Lookup debug: "+result.debug+")"):"(Lookup debug: no result)";
        setThread(t=>t.map((m,i)=>i===t.length-1?{role:"assistant",text:dbg,live:true,at:new Date()}:m));
      }
    }
