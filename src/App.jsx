...
export default function App(){
  ...
  async function runPipeline(){
    ...
    if(p.key==="blueprint"){
      ...
      const ub=await checkUsage("blueprint",{accountId:account&&account.id,admin:account&&account.admin,limit:account?(settings.blueprintFreeSignup??5):(settings.blueprintFreeNoSignup??2),windowHours:account?(settings.blueprintRefillHours||36):9999});
      if(ub&&ub.allowed===false){
        if(!account){ setPipeOut({text:"You've used your free blueprints. Sign up (it's free) to keep designing - and unlock the full create hub.",live:false,name:p.name,key:p.key,needSignup:true}); }
        else { setPipeOut({text:"You've used your blueprints for this cycle"+(ub.resetAt?" - they refill "+resetPhrase(ub.resetAt):". They refill soon")+".",live:false,name:p.name,key:p.key,capped:true}); }
        return;
      }
    }
    ...
    const maxTok=isGame?5000:(isBlueprint?5000:undefined);
  ...
  async function autoFixGame(errorMsg){
    ...
    try{
      const res=await callClaude(fixBrief,brokenCode,settings.paidModel||MODEL_PAID,5000);
      if(res&&res.text){
