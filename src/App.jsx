...
export default function App(){
  ...
  async function doCore(){
    ...
    // TEMP HARD DIAGNOSTIC: fire the lookup on EVERY answer so we can see exactly what Tavily returns.
    if(live&&settings.killSearch!==true){
      setThread(t=>[...t,{role:"assistant",text:"",lookupPending:true,at:new Date()}]);
      const result=await tryLookup(q);
      if(result&&result.answer){
        setThread(t=>t.map((m,i)=>i===t.length-1?{role:"assistant",text:"🔎 TAVILY WORKS — here's what it found:\n\n"+result.answer,live:true,at:new Date()}:m));
      }else{
        const dbg=result&&result.debug?("🔎 (Lookup debug: "+result.debug+")"):"🔎 (Lookup debug: no result object)";
