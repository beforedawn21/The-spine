...
import {
  ...
  loadWallet, setWalletBalance, addLedger, loadLedger,
...
/* Lightweight, safe markdown → React renderer for AI answers.
...
const TASKS = [
  ...
  { q:"Boiling point of water at sea level?",           a:"Water boils at 100°C at sea level.",                   gold:"good" },
...
const PIPELINES = [
  ...
  { key:"web",    name:"Website / App", icon:"⌬", plan:"Describe a site or app — Spine writes the full, working code.", brief:"You are Spine's web engineer. Build a complete, working single-file site or app for the request: real HTML/CSS/JS, clean and responsive. Return the full code in one fenced block, then a short note on structure. Production-quality, no placeholders." },
  { key:"game",   name:"Video Game",   icon:"⬢", plan:"Describe a game — Spine designs and builds it. Premium tier.", brief:"You are Spine's game engine. From the request, design a real playable game: core loop, mechanics, controls, win/lose, and a level. For simple/2D/text games, return complete runnable HTML5/JavaScript in one fenced block. For rich 3D games, return a full design + scene spec and mark it for the hosted game runtime." },
  { key:"blueprint", name:"Blueprint", icon:"⊞", plan:"Blueprints, 3D models, curriculums, experiments — any field.", brief:"You are Spine's blueprint engine for experts in any field — architects, engineers, teachers, scientists. From the request, produce a complete, structured, step-by-step plan: for a build, an ordered assembly sequence with each part and how it fits; for a curriculum, modules and lessons; for an experiment, hypothesis, materials, procedure, and safety. Be concrete and field-accurate. Note where a 3D walkthrough and an on-screen guide would accompany each step." },
  { key:"agent",  name:"Agent",       icon:"◉", plan:"Hand the spine a task; it plans the steps and runs the safe ones.", brief:"You are a Spine autonomous agent. Break the task into concrete numbered steps, mark which you can execute now (text, code, retrieval) vs. which need a connected tool or human approval, then carry out the safe steps and report results." },
];
const EARN_NERVES = [
...
async function callClaude(system,user,model){
  ...
  try{
    const res=await fetch("/api/chat",{
      ...
      signal:ctrl.signal,
      body:JSON.stringify({system:(system||"")+STYLE_RULE,user:user||"(no input)",model}),
    });
...
.topnav{display:flex;align-items:center;justify-content:space-between;padding:22px 0 20px;border-bottom:1px solid var(--stone);margin-bottom:34px;gap:20px;flex-wrap:wrap;}
...
.audit-row .av .chk::before{content:'✓ ';color:var(--live);}
...
.field input,.field select{width:100%;border:1px solid var(--stone);border-radius:8px;padding:12px 13px;background:var(--panel);font:400 14px/1.5 'Inter';color:var(--ink);transition:.14s;}
...
.field textarea{width:100%;border:1px solid var(--stone);border-radius:8px;padding:12px 13px;background:var(--panel);font:400 14px/1.5 'Inter';color:var(--ink);transition:.14s;resize:vertical;min-height:88px;}
...
.code-search{flex:1;border:none;background:transparent;resize:none;font:400 14px/1.5 'Inter';color:var(--ink);outline:none;padding:14px 14px;min-height:48px;max-height:200px;}
...
.living::before{content:"◉ Spinal insight: ";font-weight:600;}
...
.msg-user{align-self:flex-end;max-width:85%;background:var(--ink);color:var(--ivory);padding:11px 14px;border-radius:14px 14px 4px 14px;}
.msg-user,.msg-user .msg-text,.msg-user .rich,.msg-user .rich-p{color:var(--ivory);}
.msg-user .msg-text{font-size:14px;line-height:1.5;white-space:pre-wrap;}
...
.msg-ai{align-self:flex-start;max-width:92%;background:var(--panel);border:1px solid var(--stone);padding:14px 16px;border-radius:14px 14px 14px 4px;}
...
export default function App(){
  ...
  async function startCheckout(kind){
    if(!settings.subscriptionsLive&&kind==="pro"){ alert("Subscriptions are coming soon."); return; }
    try{
  ...
  async function doGenerateVideo(){
    if(vidBusy||!vidPrompt.trim())return;
    ...
    try{
      ...
      const res=await fetch("/api/video",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:vidPrompt.trim(),seconds})});
      const data=await res.json();
  ...
  useEffect(()=>{
    let alive=true;
    (async()=>{
      const st=await loadSettings(); if(alive&&st){ setSettings(st); setSettingsDraft(st); }
      ...
      if(savedId==="admin-beforedawn"){
        ...
        if(alive&&acct){
          ...
          const w=await loadWallet(acct.id); if(alive&&w!=null)setBalance(w);
  ...
  useEffect(()=>{
    ...
    if(!balanceReady.current){ balanceReady.current=true; return; }
    const id=setTimeout(()=>setWalletBalance(account.id, balance), 400);
  ...
  async function runPipeline(){
    ...
    const res=await callClaude(p.brief,pipeIn);
    setPipeOut({text:res.text,live:res.live,name:p.name,key:p.key});setPipeBusy(false);flash();
  }
  function previewWeb(){
    const code=(()=>{const m=(pipeOut?.text||"").match(/```[a-zA-Z]*\n([\s\S]*?)```/);return m?m[1]:pipeOut?.text||"";})();
    setPipeOut(o=>({...o,preview:code}));
  }
  ...
  function guard(text){
    ...
    // Sexual / drugs / crime → clever deflection, flagged to admin, kept on-platform.
  ...
  async function doCore(){
    ...
    // SMART ROUTING: simple → cheap model, coding/hard → premium. (Admins + ensemble always premium.)
    ...
    if(settings.cachingEnabled!==false&&!ensembleMode){
      try{
    ...
    // LAST-RESORT LOOKUP: if the AI admitted a gap and search is on, look it up and present in-house.
    if(live&&answerSignalsGap(answer)&&settings.searchEnabled===true&&settings.killSearch!==true){
      setThread(t=>[...t,{role:"assistant",text:"",lookupPending:true,at:new Date()}]);
      const found=await tryLookup(q);
      if(found){
        setThread(t=>t.map((m,i)=>i===t.length-1?{role:"assistant",text:"Let me check that for you — here's what I found:\n\n"+found,live:true,at:new Date()}:m));
      }else{
        setThread(t=>t.filter((m,i)=>!(i===t.length-1&&m.lookupPending)));
      }
    }
    // Cache a clean, short, live answer for identical future questions (cost saver).
    if(settings.cachingEnabled!==false&&!ensembleMode&&live&&answer&&answer.length<2000){
  ...
  function answerSignalsGap(text){
    ...
    return /\b(i don'?t (have|know)|i'm not (sure|certain)|i can'?t (be sure|verify)|not in my (training|knowledge)|don'?t have (that|exact|specific)|outside my knowledge|may be outdated|i'm unable to confirm)\b/.test(t);
  }
  async function tryLookup(q){
    if(settings.searchEnabled!==true||settings.killSearch===true) return null;
    try{
        ...
        {tab==="overview"&&(
          <div className="g2">
            ...
            <div className="panel">
              ...
              <p className="sub" style={{marginTop:14,fontSize:11.5,color:"var(--ink3)"}}>The Spine is a product of <b>Beforedawn</b>. © {new Date().getFullYear()} Beforedawn. All rights reserved.</p>
            </div>
          </div>
        )}
          ...
          <div className="g2">
            <div className="panel">
              ...
              <div className="pipe-switch" style={{marginTop:14}}>
                ...
                {PIPELINES.map(p=>(
                  <button key={p.key} className="pipe-tab" aria-pressed={pipe===p.key||undefined} onClick={()=>{setPipe(p.key);setPipeOut(null);}}>
            ...
            <div className="panel">
              ...
              <div className="code-bar">
                ...
                <button className="code-go" disabled={codeBusy||!codeIn.trim()} onClick={runCode} aria-label="Run">
                  {codeBusy?<span className="spinner" style={{width:15,height:15}}/>:"→"}
                </button>
              </div>
              {codeMode==="debug"&&(
            ...
            <div className="panel">
              ...
              <div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:14}}>
                <button className="run-btn" style={{width:"auto",padding:"11px 18px",marginTop:0}} onClick={()=>{setBuildMode("self");setDType("code");setShowTemplates(false);if(codeOut){const c=extractCode(codeOut.text);if(c)setDContent(c);}setTab("studio");setStudioView(profile?"create":"home");}}>Publish as a code tool →</button>
                <button className="run-btn ghost" style={{width:"auto",padding:"11px 18px",marginTop:0}} onClick={()=>setTab("core")}>Ask the spinal cord instead →</button>
              </div>
            </div>
              ...
              <div className="panel">
                ...
                {imgOut&&imgOut.src&&(
                  <div style={{marginTop:16}}>
                    ...
                    <a href={imgOut.src} download="spine-image.png" className="run-btn ghost" style={{marginTop:10,display:"inline-block",textDecoration:"none",textAlign:"center"}}>Download</a>
                    <button className="run-btn ghost" style={{marginTop:8,display:"inline-block"}} onClick={()=>addToProject({kind:"image",title:imgPrompt.slice(0,40)||"Image",src:imgOut.src})}>+ Add to project</button>
                  </div>
                )}
              </div>
            )}
            {pipe==="video"&&(
              <div className="panel">
                <div className="pipe-head"><span className="pipe-name serif">Video</span><span className="cred-tag" style={{background:settings.videoEnabled?"var(--live)":"var(--gold)"}}>{settings.videoEnabled?"Live":"Coming soon"}</span></div>
                <p className="sub" style={{marginTop:6}}>Describe a short clip and The Spine renders it. Capped at {Math.min(settings.videoMaxSeconds||5,5)}s to keep it fast and fair.</p>
                <div className="code-bar" style={{marginTop:12}}>
                  <textarea className="code-search" value={vidPrompt} onChange={e=>setVidPrompt(e.target.value)} placeholder="e.g. a paper boat sailing down a rainy street, cinematic" rows={2}/>
                </div>
                <button className="primary-cta" style={{marginTop:12}} disabled={vidBusy||!vidPrompt.trim()} onClick={doGenerateVideo}>{vidBusy?<><span className="spinner"/>&nbsp; Rendering…</>:"Generate video"}</button>
                ...
                {vidOut&&vidOut.url&&(
                  <div style={{marginTop:16}}>
                    ...
                    <a href={vidOut.url} download="spine-video.mp4" className="run-btn ghost" style={{marginTop:10,display:"inline-block",textDecoration:"none"}}>Download</a>
                    <button className="run-btn ghost" style={{marginTop:8,display:"inline-block"}} onClick={()=>addToProject({kind:"video",title:vidPrompt.slice(0,40)||"Video",src:vidOut.url})}>+ Add to project</button>
                  </div>
                )}
              </div>
            )}
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                <div className="code-bar" style={{marginTop:12}}>
                  ...
                  <button className="code-go" disabled={pipeBusy||!pipeIn.trim()} onClick={runPipeline} aria-label="Make">{pipeBusy?<span className="spinner" style={{width:15,height:15}}/>:"→"}</button>
                </div>
                ...
                {pipeOut&&(<>
                  ...
                  {(pipeOut.key==="web"||pipeOut.key==="game")&&pipeOut.live&&<button className="run-btn ghost" style={{marginTop:10}} onClick={previewWeb}>▶ Run it live (preview)</button>}
                  {pipeOut.preview&&<iframe title="preview" className="web-preview" sandbox="allow-scripts" srcDoc={pipeOut.preview}/>}
          ...
          <div className="g2">
            ...
            <div className="panel">
                  ...
                  {savedConvs.length===0?<div className="conv-empty">No saved chats yet. Your conversations save here automatically.</div>:
                    savedConvs.map(c=>(
                      <div key={c.id} className="conv-row">
                        ...
                        <button className="conv-del" onClick={()=>removeConv(c.id)} aria-label="Delete">×</button>
                      </div>
                    ))}
                </div>
              )}
                ...
                {thread.map((m,idx)=>(
                  m.role==="user"?(
                    ...
                    <div key={idx} className="msg msg-ai">
                      ...
                      {m.toImage&&(
                        <button className="primary-cta" style={{marginTop:12}} onClick={()=>{setTab("code");setPipe("image");}}>Open the Image studio →</button>
                      )}
                ...
                <div className="pipeline" style={{"--cordfill":`${(coreStages.filter(s=>s.status==="done").length/coreStages.length)*100}%`}}>
                  {coreStages.map((s,i)=>(
                    <div key={s.key} className={`stage ${s.status}`}>
                      <div><div className="stage-name serif">{s.name}{s.status==="active"&&<span className="spinner" style={{width:11,height:11,marginLeft:8,verticalAlign:"middle"}}/>}{s.status==="done"&&<span style={{color:"var(--gold)",marginLeft:8}}>✓</span>}</div><div className="stage-detail">{s.detail}</div></div>
                    </div>
                  ))}
                </div>
              )}
              ...
              {account&&!account.admin&&settings.creditsEnabled===true&&(
                <div className="free-hint">Each question costs {(settings.coreCostPaid||CORE_COST_PAID)*(ensembleMode?CORE_COST_ENSEMBLE_MULT:1)} credits{ensembleMode?" (high-stakes ×3)":""} · {balance} credits available.</div>
              )}
              {/* Input row */}
              {attached&&(
                <div className="attach-chip">
                  <span className="attach-name">📎 {attached.name}{attached.kind==="image"?" (image)":""}</span>
                  <button onClick={()=>setAttached(null)} aria-label="Remove">×</button>
                </div>
              )}
            ...
            <div className="earn-layout">
              <div className="panel">
                ...
                <button className="sbtn" disabled={!verdict||(lastResult&&!lastResult.low)} onClick={submitFeedback}>
                  {lastResult&&!lastResult.low?"Reviewed ✓":"Submit review"}
                </button>
                {lastResult&&(lastResult.low?(
                  ...
                  <div className="eres">
                    ...
                    <p className="er-note">Quality {Math.round(lastResult.quality*100)}% × {level.name} rate ({level.mult}×). You earned {lastResult.earned} cr on ~{lastResult.value} cr of value created — Spine keeps the spread, so it never pays out more than it makes.</p>
                  </div>
                ))}
              </div>
              <div className="panel">
                ...
                <div style={{display:"flex",alignItems:"baseline",marginTop:10}}>
                  ...
                  <span className="trust-mx mono">{level.mult}×</span>
                </div>
                  ...
                  return(
                    <div key={t.id} className="tool-card" style={isRep?{opacity:.5}:undefined}>
                      ...
                      <div className="tc-row">
                        ...
                        {hasCred&&<span className="cred-tag">✓ Verified</span>}
                      </div>
                      {isRep?(
              ...
              <div className="demand-grid" style={{marginTop:14}}>
                {DEMAND_SIGNALS.map(d=>(
                  <div key={d.query} className="demand-card">
                    ...
                    <button className="run-btn ghost" style={{marginTop:10}} onClick={()=>{if(d.build==="Blueprint"||d.build==="Agent"){setPipe(d.build.toLowerCase());setTab("code");}else{setTab("studio");setStudioView(profile?"create":"home");}}}>Build this {d.build.toLowerCase()} →</button>
                  </div>
                ))}
              </div>
            ...
            <div className="panel">
              ...
              <div className="creator-grid">
                <div className="creator-card"><span className="cc-tag dev">Developer</span><div className="cc-h serif">Upload a tool, set your price, earn per outcome</div><div className="cc-s">Write the prompt, code or workflow — or upload a file. Spine handles billing, fraud and payouts.</div>
                  ...
                  <button className="run-btn" onClick={()=>{setBuildMode("self");setTab("studio");setStudioView(profile?"create":"home");}}>Build a tool →</button>
                </div>
                <div className="creator-card"><span className="cc-tag exp">Domain Expert</span><div className="cc-h serif">We turn your expertise into a tool. You earn.</div><div className="cc-s">No code, no prompts. Spine interviews what you know and builds it. You review and publish.</div>
                  ...
                  <button className="run-btn ghost" onClick={()=>{setBuildMode("interview");setTab("studio");setStudioView(profile?"create":"home");}}>Let Spine interview me →</button>
                </div>
              </div>
            </div>
          </div>
                      ...
                      myTools.map(t=>{
                        ...
                        return(
                          <div key={t.id} className="mytool">
                            ...
                            <div style={{flex:1}}>
                              <div className="mt-n serif">{t.name}{hasCred?" ✓":""}</div>
                        ...
                        {showTemplates?(
                          <>
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                              ...
                              <button style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"var(--ink3)"}} onClick={()=>setShowTemplates(false)}>Start blank →</button>
                            </div>
                          ...
                          <>
                            ...
                            <div className="field"><label>Upload the actual tool (optional)</label>
                              <div className="upload-row">
                                ...
                                {dFile?(
                                  <span className="file-chip">{dFile.name} · {fmtSize(dFile.size)} · {dFile.runnable?"fires sandboxed":dFile.code?"code held":"stored"}<button type="button" onClick={()=>setDFile(null)}>×</button></span>
                            ...
                            {!showTest?(
                              <button className="test-trigger" onClick={()=>setShowTest(true)}>Test before publishing →</button>
                            ):(
                              <div className="test-panel">
                                <div className="test-head">
                                  <span className="th serif">Test your tool — free, no credits charged</span>
                                  <button className="tc" onClick={()=>{setShowTest(false);setTestOut(null);setTestPassed(null);}}>Done testing ×</button>
                                </div>
                                ...
                                {testOut&&(<>
                                  ...
                                  {testPassed&&<div className="outcome-done">Tested and verified ✓ — ready to publish.</div>}
                                </>)}
                              </div>
                            )}
                            <button className="primary-cta" disabled={!dName.trim()||!dDesc.trim()||(dCollab&&!dCollabH.trim())} onClick={publishTool}>
                              {testPassed?"Publish to marketplace ✓":"Publish to marketplace"}
                            </button>
              ...
              <div className="led">
                {ledger.length===0?(
                  ...
                  <div key={e.id} className="lrow">
                    <div className={`lic ${e.dir}`}>{e.dir==="in"?"↓":"↑"}</div>
                    <div>
                      <div className="lm-t">{e.title}{e.quality!=null&&` · quality ${Math.round(e.quality*100)}%`}</div>
                      <div className="lm-f">{e.from} <span className="larr">→</span> {e.to} · {e.t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit",second:"2-digit"})}</div>
                    </div>
        ...
        <div className="modal-back" onClick={()=>setSignInOpen(false)}>
          <div className="modal" style={{maxWidth:440}} onClick={e=>e.stopPropagation()}>
            <div className="modal-h">
              ...
              <button className="modal-x" onClick={()=>setSignInOpen(false)} aria-label="Close">×</button>
            </div>
        ...
        <div className="modal-back" onClick={()=>setDeleteOpen(false)}>
          <div className="modal" style={{maxWidth:440}} onClick={e=>e.stopPropagation()}>
            <div className="modal-h">
              ...
              <button className="modal-x" onClick={()=>setDeleteOpen(false)} aria-label="Close">×</button>
            </div>
            <div className="modal-body">
        ...
        <div className="modal-back" onClick={()=>{setContactOpen(false);setCSent(false);}}>
          <div className="modal" style={{maxWidth:480}} onClick={e=>e.stopPropagation()}>
            <div className="modal-h">
              ...
              <button className="modal-x" onClick={()=>{setContactOpen(false);setCSent(false);}} aria-label="Close">×</button>
            </div>
            <div className="modal-body">
              {cSent?(
        ...
        <div className="modal-back" onClick={()=>setLegalDoc(null)}>
          <div className="modal" style={{maxWidth:560}} onClick={e=>e.stopPropagation()}>
            <div className="modal-h">
              ...
              <button className="modal-x" onClick={()=>setLegalDoc(null)} aria-label="Close">×</button>
            </div>
            <div className="modal-body">
        ...
        <div className="admin-fullscreen">
          <div className="admin-fs-inner">
            <div className="modal-h admin-fs-h">
              ...
              <button className="modal-x" onClick={()=>setAdminOpen(false)} aria-label="Close">×</button>
            </div>
                ...
                <div className="bs-grid">
                      ...
                      <div className="bs-section">
                        ...
                        {ledger.slice(0,6).map((l,i)=>(
                          <div key={i} className="bs-row">
                            <div><div className="bs-rn">{l.title} <span style={{color:l.dir==="in"?"var(--live)":"var(--ink3)"}}>{l.dir==="in"?"+":"−"}{l.amt} cr</span></div><div className="bs-rd">{l.from} → {l.to}</div></div>
                          </div>
                        ))}
                  ...
                  <div className="bs-section">
                    ...
                    <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                      ...
                      <button className="bs-btn" onClick={()=>{const v=prompt("Set your displayed credits:",String(balance));if(v==null)return;const n=parseInt(v,10);if(!isNaN(n))setBalance(n);}}>Set my credits</button>
                      <button className="bs-btn alt" onClick={()=>setBalance(999999)}>Max out</button>
                    </div>
                  </div>
                  ...
                  <div className="bs-section bs-controls-section">
                    ...
                    <div className="bs-controls">
                      ...
                      <div className="bs-ctrl wide" style={{borderTop:"1px solid var(--stone)",paddingTop:10,marginTop:4}}><span style={{fontWeight:600,color:"var(--ink2)"}}>Tiers &amp; limits (off until payments connect)</span></div>
                      ...
                      <label className="bs-ctrl"><span>Free (no acct): chats</span>
                        <input type="number" min="0" value={settingsDraft.freeNoAcctChats} onChange={e=>setSettingsDraft(d=>({...d,freeNoAcctChats:parseInt(e.target.value||"0",10)}))}/></label>
                      <label className="bs-ctrl"><span>Free (no acct): images</span>
                        <input type="number" min="0" value={settingsDraft.freeNoAcctImages} onChange={e=>setSettingsDraft(d=>({...d,freeNoAcctImages:parseInt(e.target.value||"0",10)}))}/></label>
                      <label className="bs-ctrl"><span>Free acct: chats/mo</span>
                        <input type="number" min="0" value={settingsDraft.freeAcctChats} onChange={e=>setSettingsDraft(d=>({...d,freeAcctChats:parseInt(e.target.value||"0",10)}))}/></label>
                      <label className="bs-ctrl"><span>Free acct: images/mo</span>
                        <input type="number" min="0" value={settingsDraft.freeAcctImages} onChange={e=>setSettingsDraft(d=>({...d,freeAcctImages:parseInt(e.target.value||"0",10)}))}/></label>
                      <label className="bs-ctrl"><span>Free acct: videos/mo</span>
                        <input type="number" min="0" value={settingsDraft.freeAcctVideos} onChange={e=>setSettingsDraft(d=>({...d,freeAcctVideos:parseInt(e.target.value||"0",10)}))}/></label>
                      ...
                      <label className="bs-ctrl toggle"><span>Live search (Tavily)</span>
                        <input type="checkbox" checked={settingsDraft.searchEnabled} onChange={e=>setSettingsDraft(d=>({...d,searchEnabled:e.target.checked}))}/></label>
                      <label className="bs-ctrl toggle"><span>Video (Kling/Runway)</span>
                        <input type="checkbox" checked={settingsDraft.videoEnabled} onChange={e=>setSettingsDraft(d=>({...d,videoEnabled:e.target.checked}))}/></label>
                      <label className="bs-ctrl"><span>Video max seconds (cap)</span>
                        <input type="number" min="1" max="5" value={settingsDraft.videoMaxSeconds} onChange={e=>setSettingsDraft(d=>({...d,videoMaxSeconds:Math.min(5,parseInt(e.target.value||"5",10))}))}/></label>
                      <label className="bs-ctrl toggle"><span>Subscriptions live</span>
                        <input type="checkbox" checked={settingsDraft.subscriptionsLive} onChange={e=>setSettingsDraft(d=>({...d,subscriptionsLive:e.target.checked}))}/></label>
                      ...
                      <label className="bs-ctrl toggle"><span>🛑 Kill images</span>
                        <input type="checkbox" checked={settingsDraft.killImages} onChange={e=>setSettingsDraft(d=>({...d,killImages:e.target.checked}))}/></label>
                      <label className="bs-ctrl toggle"><span>🛑 Kill video</span>
                        <input type="checkbox" checked={settingsDraft.killVideo} onChange={e=>setSettingsDraft(d=>({...d,killVideo:e.target.checked}))}/></label>
                      <label className="bs-ctrl toggle"><span>🛑 Kill search</span>
                        <input type="checkbox" checked={settingsDraft.killSearch} onChange={e=>setSettingsDraft(d=>({...d,killSearch:e.target.checked}))}/></label>
                      ...
                      <div className="bs-ctrl wide" style={{borderTop:"1px solid var(--stone)",paddingTop:10,marginTop:4}}><span style={{fontWeight:600,color:"var(--ink2)"}}>Creator reward pool (money out ≤ money in)</span></div>
                    ...
                    <div style={{display:"flex",gap:10,alignItems:"center",marginTop:12,flexWrap:"wrap"}}>
                      <button className="bs-btn" onClick={async()=>{ const r=await saveSettings(settingsDraft); if(r&&r.ok){ setSettings(settingsDraft); setSettingsSaved("Saved — live now."); setTimeout(()=>setSettingsSaved(""),2500);} else { setSettingsSaved((r&&r.error)||"Couldn't save."); } }}>Save controls</button>
                  ...
                  <div className="bs-section">
                    ...
                    <p className="bs-sub">Each feature needs its API key set in Vercel AND its toggle on here. Green = toggled on. (Whether the key itself is set lives in Vercel — toggling on without a key shows users "coming soon" gracefully.)</p>
                    <div className="prov-list">
                      <div className="prov-row"><span className={"prov-dot "+(settings.imageEnabled!==false&&!settings.killImages?"on":"off")}/><span>Images (OpenAI)</span><span className="prov-state">{settings.killImages?"killed":(settings.imageEnabled!==false?"on":"off")}</span></div>
                      <div className="prov-row"><span className={"prov-dot "+(settings.searchEnabled&&!settings.killSearch?"on":"off")}/><span>Live search (Tavily)</span><span className="prov-state">{settings.killSearch?"killed":(settings.searchEnabled?"on":"off")}</span></div>
                      <div className="prov-row"><span className={"prov-dot "+(settings.videoEnabled&&!settings.killVideo?"on":"off")}/><span>Video (Kling/Runway)</span><span className="prov-state">{settings.killVideo?"killed":(settings.videoEnabled?"on":"off")}</span></div>
                      <div className="prov-row"><span className={"prov-dot "+(settings.subscriptionsLive?"on":"off")}/><span>Payments (Stripe)</span><span className="prov-state">{settings.subscriptionsLive?"live":"coming soon"}</span></div>
                    </div>
                  </div>
        ...
        <div className="modal-back" onClick={closeRun}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-h">
              ...
              <button className="modal-x" onClick={closeRun} aria-label="Close">×</button>
            </div>
            <div className="modal-body">
              <div className="run-meta">
                ...
                {credScore(runTool)&&<span className="rm" style={{color:"var(--live)"}}>✓ Verified tool</span>}
              </div>
