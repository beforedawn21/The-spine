...
export default function App(){
  ...
  async function runPipeline(){
    ...
    if(p.key==="blueprint"&&res.live&&settings.killImages!==true){
      ...
      // The image generator draws far better from a tight VISUAL description than from a wall of specs
      // or a vague one-liner. Ask the model to distill the spec it just wrote into a focused drawing prompt
      // that names the object, its overall shape/proportions, the key labeled parts, and which views to show.
      let drawPrompt="";
      try{
        const distill=await callClaude(
          "You write image-generation prompts for technical blueprint drawings. Given a build spec, output ONE concise prompt (max 60 words) describing a clean blueprint drawing of THIS specific object: its overall form and proportions, 2-3 key labeled parts with their approximate sizes, and which orthographic views to show (front/side/top). Name real shapes and parts. Output ONLY the prompt text, nothing else.",
          "Build request: "+pipeIn.trim()+"\n\nSpec:\n"+String(res.text||"").slice(0,1500)
        );
        if(distill&&distill.text){ drawPrompt=distill.text.trim(); }
      }catch(e){}
      if(!drawPrompt){ drawPrompt="technical blueprint drawing of "+pipeIn.trim()+", labeled parts with dimensions, orthographic front/side/top views"; }
      // Wrap with consistent blueprint styling so the output reads as a real drafting sheet.
      const styled=drawPrompt+". Drafting blueprint style: crisp white line-art on deep blueprint-blue background, clean dimension lines with measurement callouts, labeled parts, orthographic projection, technical drawing, no photorealism, no people.";
      const imgSrc=await pullImage(styled,true);
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      +'<style>html,body{margin:0;padding:0;width:100vw;height:100vh;min-height:460px;overflow:hidden;background:#000;}canvas{display:block;margin:0 auto;}#__status{position:fixed;top:0;left:0;right:0;z-index:99999;background:rgba(0,0,0,.85);color:#0f0;font:12px/1.4 monospace;padding:6px 10px;}</style>'
      +'</head><body>'
      ...
      +'try{var __d=__dim();'
      +'Object.defineProperty(window,"innerWidth",{configurable:true,get:function(){return __dim().w;}});'
      +'Object.defineProperty(window,"innerHeight",{configurable:true,get:function(){return __dim().h;}});'
      +'}catch(e){}'
      ...
      // Watchdog: confirm a real, non-zero canvas exists. If none, the game silently failed -> auto-fix.
      +'setTimeout(function(){if(!__s)return;if(__s.textContent.indexOf("error")>=0)return;'
      +'var cv=document.querySelector("canvas");'
      +'if(cv&&cv.width>1&&cv.height>1){__log("game started OK","#0f0");try{parent.postMessage({__spineGame:true,ok:true},"*");}catch(e){}setTimeout(function(){if(__s)__s.style.display="none";},1800);}'
