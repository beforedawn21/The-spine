...
const PIPELINES = [
  ...
  { key:"game",   name:"Video Game",   icon:"⬢", plan:"Describe a game - Spine designs and builds it, playable right here.", brief:"You are Spine's game engine. Design and BUILD a real, playable game. Return complete, runnable, polished HTML5/JavaScript in one fenced block - working touch controls, score, win/lose, clean visuals, playable immediately in a browser. Include sound where it helps (Web Audio). Make it actually fun, not a tech demo." },
...
export default function App(){
  ...
  const [gameMode,setGameMode]=useState("canvas"); // "canvas" (pure 2D canvas - iOS-safe default) | "2d" (Phaser) | "3d" (Three.js)
  ...
  function publishToolToMarket(){
    if(!pipeOut||pipeOut.key!=="tool"||!pipeOut.text) return;
  ...
  function saveAssistant(){
    ...
    setSavedAssistants(list=>[{id,name,desc,instruction,full:t,at:Date.now()},...list.filter(a=>a.name!==name)].slice(0,40));
  ...
  function saveTool(){
    ...
    setSavedTools(list=>[{id,name,code:pipeOut.text,at:Date.now()},...list.filter(t=>t.name!==name)].slice(0,40));
  ...
  function openSavedTool(t){
    ...
    setTab("code"); setPipe("tool"); setPipeIn(t.name);
    setPipeOut({text:t.code,live:true,name:"Tool",key:"tool",preview:buildToolDoc(t.code)});
  }
  async function refineTool(){
    if(!pipeOut||pipeOut.key!=="tool"||!refineReq.trim()||pipeBusy) return;
    setPipeBusy(true); setToolErr(null);
    const p=PIPELINES.find(x=>x.key==="tool");
    ...
    if(res&&res.live&&res.text){
      setPipeOut(o=>({...o,text:res.text,preview:buildToolDoc(res.text)}));
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {(pipeOut.key==="web"||pipeOut.key==="game"||pipeOut.key==="tool")&&pipeOut.live&&<button className="run-btn ghost" style={{marginTop:10}} onClick={previewWeb}>{"\u25b6"} {pipeOut.key==="game"?"Play game":pipeOut.key==="tool"?"Run this tool":pipeOut.key==="world"?"Enter the world":"Run it live (preview)"}</button>}
                  ...
                  {pipeOut.key==="tool"&&pipeOut.preview&&(
                    <>
                      <div className="tool-acts">
