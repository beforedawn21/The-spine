...
LIVE DATA - YOU CAN PULL IT: You DO have a live feed to the outside world. The Spine searches the web for you, and you can read any page a person pastes. So for current things - news, weather, prices, scores, "what's the latest" - you CAN get real, current information, and you should. Never say you're unable to see live data; never say things like "I'm the reasoning, not the radar." Answer with the real, current facts, delivered with your usual warmth. The ONE rule: never fabricate. If a lookup genuinely comes back empty, say plainly that the feed came back empty just now, don't invent a number, and offer the thinking you CAN do (context, trend, what usually moves it) while they retry.
...
export default function App(){
  ...
  useEffect(()=>{
    try{
      if(thread&&thread.length){ const slim=thread.slice(-20).map(m=>({role:m.role,text:m.text,live:m.live,imgSrc:m.imgSrc,sources:m.sources,at:(m.at&&m.at.getTime?m.at.getTime():Date.now())})); localStorage.setItem("spine.thread",JSON.stringify(slim)); }
  ...
  useEffect(()=>{
    function onMsg(e){
      ...
      if(d&&d.__spineTool){
        ...
        if(d.kind==="save"){
          try{ const b=bag(); b[slot]=b[slot]||{}; b[slot][p.key]=p.value; localStorage.setItem("spine.tooldata",JSON.stringify(b)); reply({ok:true}); }catch(_){ reply({ok:false}); }
          return;
        }
  ...
  function recallMemory(query){
    try{
      ...
      const qw=new Set(query.toLowerCase().replace(/[^a-z0-9\s]/g," ").split(/\s+/).filter(w=>w.length>4));
      const scored=mems.map(m=>{ let s=0; (m.topics||[]).forEach(t=>{ if(qw.has(t))s+=2; }); const days=(Date.now()-m.at)/86400000; s+=Math.max(0,2-days*0.1); return {m,s}; })
        .filter(x=>x.s>1).sort((a,b)=>b.s-a.s).slice(0,3);
                  ...
                  <div style={{marginTop:10}}>
                    {[
                      {k:"real", icon:"\uD83C\uDF0D", name:"Real places", blurb:"Built from real data, not invented",
                       ex:["The night sky above me right now","My street in 3D","Stand on top of Everest","The solar system, to scale","Where the ISS is passing overhead"]},
                      {k:"body", icon:"\uD83E\uDEC0", name:"Inside the body", blurb:"Anatomy you can stand inside",
                       ex:["Inside a heart with a blocked left artery and a stent","Inside a lung during an asthma attack","Stand inside a human cell","Follow a blood cell through an artery","Inside the inner ear"]},
                      {k:"space",icon:"\uD83C\uDFDB", name:"Spaces & plans", blurb:"Walk it before it exists",
                       ex:["Walk through this floor plan","A kitchen with an island and a window over the sink","Inside a cathedral","My garden redesigned"]},
                      {k:"calm", icon:"\uD83C\uDF32", name:"Places to be", blurb:"Somewhere to stand and breathe",
                       ex:["A calm forest at night with fireflies","A beach at sunrise, waves rolling in","Falling snow in a quiet street","Underwater on a coral reef"]},
                      {k:"learn",icon:"\uD83D\uDD2C", name:"Learn by standing in it", blurb:"The lesson, at full size",
                       ex:["Stand inside a water molecule","Inside a volcano as it erupts","A cross-section of the Earth","Walk through the Battle of Hastings"]},
                    ].map(sec=>(
                  ...
                  <div style={{marginTop:10}}>
                    {[
                      {k:"cam",  icon:"\uD83D\uDCF7", name:"Camera",    blurb:"Reads and recognises what you point it at",
                       ex:["Measure something using a coin or card for scale",
                ...
                {thread.length===0&&!coreBusy&&(
                  ...
                  <div className="greet">
                    ...
                    <div className="qa-chips">
                      {[["Summarize","Summarize this clearly: "],["Translate","Translate this (tell me the language): "],["Rewrite","Rewrite this to be clearer: "]].map(([label,pre],i)=>(
                        <button key={i} className="qa-chip" onClick={()=>setCoreReq(pre)}>{label}</button>
                      ))}
                    </div>
                    <div className="starters">
                      {["Explain a complex topic simply","Research something deeply","Create an image, video or song"].map((s,i)=>(
                        <button key={i} className="starter" onClick={()=>doCore(s)}>{s}</button>
                      ))}
                      <button className="starter" onClick={()=>{setTab("code");setPipe("tool");setPipeOut(null);}}>{"\uD83D\uDEE0"} Build me a tool</button>
                    </div>
                  </div>
                  </>
                )}
