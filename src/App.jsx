...
export default function App(){
  ...
  async function doCore(){
    ...
    // LAST-RESORT LOOKUP: if the AI admitted a gap, try a lookup. Activates automatically when the Tavily key is set.
    if(live&&answerSignalsGap(answer)&&settings.killSearch!==true){
  ...
  async function tryLookup(q){
    if(settings.killSearch===true) return null; // kill switch still respected
    try{
      const res=await fetch("/api/search",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:q})});
      const data=await res.json();
      if(data.notConnected||data.error) return null; // no key = silently skip
