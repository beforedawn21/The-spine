...
async function callClaude(system,user,model,maxTokens){
  ...
  const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),big?55000:28000);
  try{
...
export default function App(){
  ...
  async function runPipeline(){
    ...
    const res=await callClaude(brief,promptToSend,gameModel,isGame?8000:undefined);
  ...
  async function autoFixGame(errorMsg){
    ...
    try{
      const res=await callClaude(fixBrief,brokenCode,settings.paidModel||MODEL_PAID,8000);
      if(res&&res.text){
