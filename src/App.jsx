...
.clarify-input{width:100%;box-sizing:border-box;background:var(--panel);border:1px solid var(--stone);border-radius:8px;padding:10px;font:500 13px/1.4 'Inter';color:var(--ink);resize:vertical;}
...
export default function App(){
  ...
  async function runPipeline(){
    ...
    if(p.key==="game"){
      ...
      if(gameMode==="3d"){
        brief="You are Spine's 3D game engine. Build a complete, playable 3D game using Three.js. Assume THREE is already loaded as a global (do NOT add a script tag or import for it - just use `THREE`). Return ONE complete HTML5/JavaScript block: a full <body> with a canvas/renderer, a real game loop, lighting, camera, player movement, win/lose, and score. CRITICAL - THIS RUNS ON A PHONE (touchscreen, no keyboard): the game MUST start on a TAP/CLICK anywhere, never on a key press. Do NOT write 'press space' or 'press enter' to start - use a tappable on-screen Start button or 'tap to start', and listen for BOTH 'click' and 'touchstart' events. Every core action (move, jump, shoot, etc.) MUST be playable by touch via on-screen buttons or tap/drag/swipe on the canvas. Keyboard may be an OPTIONAL extra, never the only way to do anything. Add a short title/instructions overlay that starts on tap. Make it genuinely playable immediately on a phone. Keep it self-contained in one file. IMPORTANT: keep the code COMPACT and COMPLETE - close every function and return complete, runnable code.";
      }else{
        brief="You are Spine's 2D game engine. Build a complete, playable 2D game using Phaser 3. Assume Phaser is already loaded as a global (do NOT add a script tag or import for it - just use `Phaser`). Return ONE complete HTML5/JavaScript block: a full Phaser.Game config with preload/create/update, real sprites or graphics, physics, working controls, score, and win/lose. CRITICAL - THIS RUNS ON A PHONE (touchscreen, no keyboard): the game MUST start on a TAP/POINTER anywhere, never on a key press. Do NOT write 'press space' or 'press enter' to start - use a tappable Start button or 'tap to start' and use Phaser's pointerdown/pointerup input. Every core action (move, jump, shoot, etc.) MUST be playable by touch - use on-screen buttons or pointer/drag/swipe on the canvas. Keyboard may be an OPTIONAL extra, never the only way to do anything. Set Phaser scale to fit the screen (scale.mode = Phaser.Scale.FIT, autoCenter). Add a short title/instructions overlay that starts on tap. Make it actually fun and playable immediately on a phone. Keep it self-contained in one file. IMPORTANT: keep the code COMPACT and COMPLETE - close every function and return complete, runnable code.";
      }
    }
    ...
    const res=await callClaude(brief,promptToSend,gameModel,isGame?3000:undefined);
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    // Engine sources, tried in order: YOUR OWN domain first (bulletproof, no external dep),
    // then public CDNs as a fallback so games still work even before the self-hosted file is added.
    const cdns=is3d
    ...
    // Strip any engine <script src=...> the AI added (we control load order + sources).
    code=code.replace(/<script[^>]*src=["'][^"']*(phaser|three)[^"']*["'][^>]*>\s*<\/script>/gi,"");
    // Pull out inline scripts so they run AFTER the engine is ready.
    let inlineJs="";
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      +'<style>html,body{margin:0;padding:0;width:100%;height:100%;min-height:460px;overflow:hidden;background:#000;}canvas{display:block;margin:0 auto;}#__status{position:fixed;top:0;left:0;right:0;z-index:99999;background:rgba(0,0,0,.85);color:#0f0;font:12px/1.4 monospace;padding:6px 10px;}</style>'
      +'</head><body>'
      ...
      +'try{if(!window.innerWidth||window.innerWidth<50){Object.defineProperty(window,"innerWidth",{configurable:true,get:function(){return document.documentElement.clientWidth||400;}});}'
      +'if(!window.innerHeight||window.innerHeight<50){Object.defineProperty(window,"innerHeight",{configurable:true,get:function(){return document.documentElement.clientHeight||600;}});}}catch(e){}'
      ...
      // Tap-to-key bridge: many AI games gate "start" behind a keypress (space/enter) that a phone
      // can't send. On any tap/touch, synthesize the common start/action keys so the game responds.
      +'(function(){function k(type,key,code,kc){try{var e=new KeyboardEvent(type,{key:key,code:code,keyCode:kc,which:kc,bubbles:true,cancelable:true});document.dispatchEvent(e);window.dispatchEvent(e);if(document.body)document.body.dispatchEvent(e);}catch(_){}}'
      +'function tap(){[[" ","Space",32],["Enter","Enter",13],["ArrowUp","ArrowUp",38]].forEach(function(a){k("keydown",a[0],a[1],a[2]);setTimeout(function(){k("keyup",a[0],a[1],a[2]);},60);});}'
      +'window.addEventListener("touchstart",tap,{passive:true});window.addEventListener("pointerdown",tap);})();'
      // Watchdog: a real game draws a <canvas>. After a moment, if no canvas exists (or it is 0-size),
      // the game silently failed to start - report "stuck" so the app can auto-fix it.
      +'setTimeout(function(){if(!__s)return;if(__s.textContent.indexOf("error")>=0)return;'
      +'var cv=document.querySelector("canvas");'
      +'if(cv&&cv.width>0&&cv.height>0){__log("game started OK","#0f0");try{parent.postMessage({__spineGame:true,ok:true},"*");}catch(e){}setTimeout(function(){if(__s)__s.style.display="none";},2000);}'
      +'else{__log("game did not start (no visible canvas)","#fa0");try{parent.postMessage({__spineGame:true,error:"Game ran but produced no visible canvas - the game did not actually start. Check the scene/start logic and ensure a Phaser.Game/THREE renderer canvas is created and added to the page."},"*");}catch(e){}}'
      +'},1200);'
  ...
  // iOS Safari does NOT reliably run scripts inside a sandboxed srcDoc iframe - the static HTML shows
  // but the script never executes (stuck on "starting..."). Loading the doc as a Blob URL via src gives
  // it a real origin where scripts run reliably. This is the fix for the frozen game screen.
  function docToBlobUrl(doc){
    try{ return URL.createObjectURL(new Blob([doc],{type:"text/html"})); }catch(e){ return null; }
  }
  function previewWeb(){
    let doc, blobUrl=null;
    try{ if(pipeOut&&pipeOut.previewUrl) URL.revokeObjectURL(pipeOut.previewUrl); }catch(e){}
    if(pipeOut&&pipeOut.key==="game"){
      ...
      blobUrl=docToBlobUrl(doc);
    }else{
      doc=(()=>{const m=(pipeOut?.text||"").match(/```[a-zA-Z]*\n([\s\S]*?)```/);return m?m[1]:pipeOut?.text||"";})();
    }
    setPipeOut(o=>({...o,preview:doc,previewUrl:blobUrl}));
  }
  ...
  async function autoFixGame(errorMsg){
    ...
    try{
      ...
      if(res&&res.text){
        setPipeOut(o=>({...o,text:res.text}));
        const fixedDoc=buildGameDoc(res.text,pipeOut?.gameMode);
        const fixedUrl=docToBlobUrl(fixedDoc);
        gameFixRef.current.fixing=false;
        setPipeOut(o=>({...o,text:res.text,preview:fixedDoc,previewUrl:fixedUrl}));
  ...
  function playMarketGame(t){
    ...
    setPlayingGame({title:t.name,doc,url});
  ...
  return(
    <div className="spa">
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {pipeOut.preview&&<iframe title="preview" className="web-preview" style={pipeOut.key==="game"?{height:480}:undefined} sandbox="allow-scripts allow-pointer-lock" srcDoc={pipeOut.preview}/>}
      ...
      {playingGame&&(
        <div className="modal-back" onClick={()=>{try{if(playingGame.url)URL.revokeObjectURL(playingGame.url);}catch(e){}setPlayingGame(null);}}>
          <div className="modal play-modal" onClick={e=>e.stopPropagation()}>
            <div className="play-head"><span className="play-title serif">{playingGame.title}</span><button className="play-x" onClick={()=>{try{if(playingGame.url)URL.revokeObjectURL(playingGame.url);}catch(e){}setPlayingGame(null);}} aria-label="Close">x</button></div>
