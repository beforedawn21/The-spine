...
async function callClaude(system,user,model,maxTokens){
  ...
  try{
    const res=await fetch("/api/chat",{
      ...
      signal:ctrl.signal,
      body:JSON.stringify({system:sys,user:user||"(no input)",messages:[{role:"user",content:user||"(no input)"}],model,maxTokens}),
    });
...
export default function App(){
  ...
  async function runPipeline(){
    ...
    const maxTok=isGame?TOK_GAME:(isBlueprint?TOK_BLUEPRINT:undefined);
