...
WHEN YOU CAN'T PULL LIVE DATA: You have no live feed to the outside world — no live news, weather, stock prices, or sports scores beyond your training. Never fabricate live data. Instead be clever and honest, then pivot to being useful: admit you can't see it from where you sit ("I'm the reasoning, not the radar"), and offer what you CAN do or where they could check. Stay charming, never a cold error.
...
TONE: Friendly, intelligent, occasionally playful. Lead with the answer. Short paragraphs.`;
...
export default function App(){
  ...
  function detectCreateIntent(text){
    ...
    // GUARDRAIL 1: ignore capability questions ("do you make pictures?", "can you draw?", "what images can you make")
    const isQuestion=/\?\s*$/.test(raw)||/^(can|could|do|does|are|is|will|would|how|what|whats|what's|why|when|where|which|who)\b/.test(t);
    // A clear, imperative create command, optionally with polite lead-ins.
    const imageCmd=/^(please\s+|hey\s+|can you\s+|could you\s+|would you\s+|i want you to\s+|i'?d like you to\s+)?(draw|sketch|paint|generate|create|make|render|design|illustrate|produce|whip up|cook up)\b/;
    // What signals it's specifically an IMAGE (not code/text)
    const imageNoun=/\b(image|images|picture|pictures|pic|pics|photo|photos|drawing|art|artwork|illustration|logo|poster|painting|portrait|wallpaper|graphic|sketch|icon|avatar|emoji|meme|comic|cartoon|sticker|scene|landscape)\b/;
    const ofPattern=/\b(image|picture|photo|drawing|illustration|logo|poster|painting|portrait|sketch|art|graphic|icon|avatar|wallpaper) of \b/;

    // IMAGE: must be a command (not a question) AND clearly about an image, with a subject after it.
    if(!isQuestion){
      // "draw me a cat", "make a picture of a sunset", "create a logo for..."
      if(imageCmd.test(t)&&(imageNoun.test(t)||/^(please\s+|hey\s+)?(draw|sketch|paint|illustrate)\b/.test(t))){
        // require something to draw (a subject) — at least a few chars after the verb
        if(raw.split(/\s+/).length>=3||ofPattern.test(t)) return {kind:"image"};
      }
      if(ofPattern.test(t)&&/^(please\s+)?(draw|make|create|generate|render|paint|sketch|design|give me|show me)\b/.test(t)) return {kind:"image"};
    }

    // MUSIC (command, not question)
    if(!isQuestion&&/^(please\s+)?(write|make|generate|compose|create|produce)\b/.test(t)&&/\b(song|music|melody|track|beat|tune|jingle|instrumental)\b/.test(t)) return {kind:"music"};
    // VIDEO
    if(!isQuestion&&/^(please\s+)?(make|generate|create|produce)\b/.test(t)&&/\b(video|animation|clip|movie|film|gif)\b/.test(t)) return {kind:"video"};
    // VOICE
    if(!isQuestion&&/\b(clone|dub|narrate|voiceover|read aloud|text to speech)\b/.test(t)&&/\b(voice|audio|speech|narration)\b/.test(t)) return {kind:"voice"};
    return null;
  }
  const MEDIA_SOON={
  ...
  async function doCore(){
    ...
    const questionCost=(account&&!isAdmin&&creditsOn)?(settings.coreCostPaid||CORE_COST_PAID)*(ensembleMode?CORE_COST_ENSEMBLE_MULT:1):0;
    ...
    // Create-intent routing: if the message is really a "make an image/song/video" request, handle it here.
    const intent=detectCreateIntent(q);
    if(intent){
      if(intent.kind==="image"&&settings.imageEnabled!==false){
        // Generate an image inline in the chat
        setCoreReq("");
        const userMsg={role:"user",text:q,at:new Date()};
        const baseT=[...thread,userMsg,{role:"assistant",text:"",image:"pending",at:new Date()}];
        setThread(baseT);
        const rem=imageRemaining();
        if(!isAdmin&&rem<=0){
          const msg=!account?"You've used your free images. Sign up to create more free pictures.":"You've used your images for now — they refill every "+(settings.imageRefillHours||36)+" hours.";
          setThread(t=>t.map((m,i)=>i===t.length-1?{...m,image:null,text:msg,live:true}:m));
          return;
        }
        try{
          const model=imgHiQuality?(settings.imageModelHi||"gpt-image-1"):(settings.imageModel||"gpt-image-1-mini");
          const res=await fetch("/api/image",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:q,model})});
          const data=await res.json();
          if(data.notConnected){ setThread(t=>t.map((m,i)=>i===t.length-1?{...m,image:null,text:"Image generation is coming online soon — the spinal cord is still wiring up its visual cortex.",live:true}:m)); return; }
          if(data.error){ setThread(t=>t.map((m,i)=>i===t.length-1?{...m,image:null,text:"Couldn't make that image: "+data.error,live:true}:m)); return; }
          const src=data.url?data.url:(data.b64?("data:image/png;base64,"+data.b64):null);
          if(!src){ setThread(t=>t.map((m,i)=>i===t.length-1?{...m,image:null,text:"No image came back. Try again.",live:true}:m)); return; }
          setThread(t=>t.map((m,i)=>i===t.length-1?{...m,image:src,text:"",live:true}:m));
          if(!isAdmin){ const nu=imgUsed+1; setImgUsed(nu); if(account&&nu>=imageLimit()&&!imgRefillAt) setImgRefillAt(Date.now()+(settings.imageRefillHours||36)*3600000); }
        }catch(e){ setThread(t=>t.map((m,i)=>i===t.length-1?{...m,image:null,text:"Image service error. Please try again.",live:true}:m)); }
        return;
      }
      if(MEDIA_SOON[intent.kind]){
        setCoreReq("");
        setThread(t=>[...t,{role:"user",text:q,at:new Date()},{role:"assistant",text:MEDIA_SOON[intent.kind],live:true,at:new Date()}]);
        return;
      }
    }
    setCoreReq(""); setShowAudit(false); setEnsembleOuts([]); setCoreBusy(true); stopRef.current=false;
    ...
    const chosenModel=account?(settings.paidModel||MODEL_PAID):(settings.freeModel||MODEL_FREE);
  ...
  async function doGenerateImage(){
    ...
    try{
      ...
      setSignalsAbsorbed(n=>n+1);
      if(!isAdmin){
        const nu=imgUsed+1; setImgUsed(nu);
  ...
  function siSignIn(){
    ...
    // Exact admin credentials open the Brain Stem directly.
    if(isAdminLogin(siName.trim(),siPass)){
      applyAuth({id:"admin-beforedawn",name:ADMIN_USER,verified:true,admin:true});
      setBalance(999999);
      return;
    }
    (async()=>{
  ...
  async function applyAuth(acct, starter){
    setAccount(acct);
    saveSession(acct.id);
    if(acct.admin) setAdminAuthed(true);
      ...
      <div className="shell">
        ...
        {tab==="code"&&(
          <div className="g2">
            ...
            {pipe!=="code"&&pipe!=="image"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {thread.map((m,idx)=>(
                  m.role==="user"?(
                    ...
                    <div key={idx} className="msg msg-ai">
                      ...
                      {m.image==="pending"?(
                        <div className="act" style={{display:"flex",alignItems:"center",gap:10,color:"var(--ink2)"}}><span className="spinner"/> Rendering your image…</div>
                      ):m.image?(
                        <div>
                          <img src={m.image} alt="Generated" style={{width:"100%",borderRadius:12,border:"1px solid var(--stone)"}}/>
                          <a href={m.image} download="spine-image.png" className="run-btn ghost" style={{marginTop:10,display:"inline-block",textDecoration:"none"}}>Download</a>
                        </div>
                      ):(
