            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {(pipeOut.key==="web"||pipeOut.key==="game"||pipeOut.key==="tool")&&pipeOut.live&&<button className="run-btn ghost" style={{marginTop:10}} onClick={previewWeb}>{"\u25b6"} {pipeOut.key==="game"?"Play game":pipeOut.key==="tool"?"Run this tool":"Run it live (preview)"}</button>}
                  {pipeOut.preview&&<iframe title="preview" className="web-preview" style={pipeOut.key==="game"||pipeOut.key==="tool"?{height:"78vh",maxHeight:"680px"}:undefined} sandbox="allow-scripts allow-pointer-lock allow-forms allow-modals allow-downloads" allow="camera; microphone; geolocation; accelerometer; gyroscope; magnetometer; clipboard-write" srcDoc={pipeOut.preview}/>}
      ...
      {playingGame&&(
        <div className="modal-back" onClick={()=>setPlayingGame(null)}>
          <div className="modal play-modal" onClick={e=>e.stopPropagation()}>
            ...
            <iframe title="play" className="play-frame" sandbox="allow-scripts allow-pointer-lock allow-forms allow-modals allow-downloads" allow="camera; microphone; geolocation; accelerometer; gyroscope; magnetometer; clipboard-write" srcDoc={playingGame.doc}/>
          </div>
        </div>
      )}
      {signInOpen&&(
