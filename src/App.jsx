...
export default function App(){
  ...
  async function doCore(override){
    ...
    const factualLookup=/\b(frame data|stats|statistics|price|cost|score|standings|ranking|release date|weather|temperature|forecast|who (is|won|plays|owns)|what (year|time|day)|when (did|is|was)|how many|how much|net worth|population of|capital of|ceo of|founded|located|schedule|lineup|roster|box office|record for)\b/i.test(q);
    const needsLookup=live&&!hasVision&&(answerSignalsGap(answer)||timeSensitive||factualLookup)&&settings.killSearch!==true;
    if(needsLookup){
      ...
      if(result&&result.answer){
        ...
        try{
          const sysL=CORE_SYS+langDirective+recallMemory(q||"")+"\n\nYou just retrieved fresh live information. Write ONE clear, final answer to the user's question using ONLY these facts. Do not say you cannot answer - you now have the information. Give the real numbers/facts accurately and up front, and keep your usual warm, clear voice with its quiet spark - a little life and a human touch, never a flat data readout, but calm and unshowy. Never fabricate anything beyond what the facts below say.\n\nLIVE FACTS:\n"+result.answer;
  ...
  function splitLookupParts(q){
    ...
    parts=parts.map(p=>{
      ...
      if(lastTopic) return lastTopic+" "+p;
      return p;
    });
    const seen={},out=[];
  ...
  async function tryLookup(q){
    ...
    const parts=splitLookupParts(q);
