...
export default function App(){
  ...
  async function autoFixGame(errorMsg){
    ...
    try{
      const res=await callClaude(fixBrief,brokenCode,settings.paidModel||MODEL_PAID,TOK_GAME_FIX);
      if(res&&res.text){
  ...
  async function doCore(){
    ...
    const history=baseThread.slice(-20).map(m=>({role:m.role,content:m.sendText||m.text}));
