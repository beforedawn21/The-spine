...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      +'setTimeout(function(){if(__s&&__s.textContent.indexOf("error")<0){__log("game started OK","#0f0");try{parent.postMessage({__spineGame:true,ok:true},"*");}catch(e){}setTimeout(function(){if(__s)__s.style.display="none";},2000);}},300);'
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {pipeOut.preview&&(pipeOut.key==="game"&&pipeOut.previewUrl
                    ? <iframe title="preview" className="web-preview" style={{height:480}} sandbox="allow-scripts allow-pointer-lock allow-same-origin" src={pipeOut.previewUrl}/>
                    : <iframe title="preview" className="web-preview" style={pipeOut.key==="game"?{height:480}:undefined} sandbox="allow-scripts allow-pointer-lock" srcDoc={pipeOut.preview}/>)}
      ...
      {playingGame&&(
        <div className="modal-back" onClick={()=>{try{if(playingGame.url)URL.revokeObjectURL(playingGame.url);}catch(e){}setPlayingGame(null);}}>
          <div className="modal play-modal" onClick={e=>e.stopPropagation()}>
            ...
            {playingGame.url
              ? <iframe title="play" className="play-frame" sandbox="allow-scripts allow-pointer-lock allow-same-origin" src={playingGame.url}/>
