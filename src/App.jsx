...
async function callClaudeChatStream(system,messages,model,onChunk){
  ...
  try{
    ...
    clearTimeout(timer);
    try{ return await callClaudeChat(system,messages,model); }catch(_){ return{text:"Sorry - the spinal cord can't reach the reasoning center right now. Please try again in a moment.",live:false}; }
  }
}
async function callClaudeChat(system,messages,model){
  ...
  try{
    ...
    if(data&&data.error){ const em=(typeof data.error==="string"?data.error:(data.error.message||JSON.stringify(data.error))).slice(0,180); return{text:hasImg?("Couldn't read the image: "+em):PREVIEW,live:false}; }
    const text=(data.content||[]).map(b=>b.type==="text"?b.text:"").join("").trim();
