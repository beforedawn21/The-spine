...
export default function App(){
  ...
  async function autoFixGame(errorMsg){
    ...
    try{
      const res=await callClaude(fixBrief,brokenCode);
      if(res&&res.text){
  ...
  async function doCore(){
    ...
    if(needsLookup){
      ...
      // Lookup found nothing - fall through to the model's own best answer (single message).
    }
  ...
  function postGameToMarket(price){
    ...
    setTools(prev=>[{id:localId,name:title,by:profile?profile.handle:(account.name||"me"),type:"game",
      ...
      gameMode:pipeOut.gameMode||"2d",playable:true,contributors:sole(profile?profile.handle:(account.name||"me")),
