...
const PIPELINES = [
  ...
  { key:"tool",   name:"Tool",        icon:"🛠", plan:"Describe a real working mini-app - a calculator, tracker, scanner, tuner - Spine builds it, running right here.", brief:"You are Spine's tool engine. BUILD a real, working single-file mini-app (a utility, calculator, tracker, form, quiz, converter, timer, scanner, logger - whatever the user describes). Return ONE complete HTML/JS block that runs immediately in a sandboxed iframe on a phone. REQUIREMENTS: (1) clean, modern, touch-friendly UI that fills the screen, big tap targets, readable fonts, a warm off-white or dark theme; (2) fully functional - real logic, real calculations, real state, no placeholders or TODOs; (3) if it's useful, PERSIST data with a simple in-memory JS object during the session (do NOT use localStorage - it is blocked in the sandbox); (4) works OFFLINE - no external scripts, fonts, or network calls. THE PHONE'S HARDWARE IS YOURS - these tricks all work in the sandbox and you should reach for them whenever the request calls for one (always request permission gracefully and stay useful if denied): [SEE] CAMERA via navigator.mediaDevices.getUserMedia({video:{facingMode:'environment'}}) into a <video> + <canvas> - build scanners, document capture, color pickers (read pixel under a crosshair), magnifiers, timelapse. [HEAR] MICROPHONE via getUserMedia({audio:true}) + AudioContext + AnalyserNode - build instrument tuners (autocorrelation pitch on getFloatTimeDomainData), decibel/noise meters (RMS of the signal), clap detectors, voice-controlled timers; this runs fully offline in the browser. [MOVE] MOTION/ORIENTATION via DeviceOrientationEvent/DeviceMotionEvent (call DeviceOrientationEvent.requestPermission?.() inside a tap on iOS) - build bubble levels, compasses, protractors, step counters, shake-to-roll dice, posture/balance trainers. [KNOW WHERE] LOCATION via navigator.geolocation - parking-spot savers, distance trackers, sunrise/sunset, 'how far am I from X'. [FEEL] HAPTICS via navigator.vibrate patterns - metronomes you feel, interval timers, feedback. [TALK] VOICE OUT via speechSynthesis + VOICE IN via webkitSpeechRecognition - hands-free recipe readers, spoken flashcards, talking coaches. [STAY ON] WAKE-LOCK via navigator.wakeLock.request('screen') for recipe/workout/teleprompter tools. [SHARE] navigator.share + navigator.clipboard.writeText for output. COMBINE senses for the best tools (camera+voice = point-and-hear-it-read; motion+haptics = a level that buzzes when true). Detect support before using each (if('vibrate' in navigator)...) and degrade gracefully. Make it genuinely useful and delightful, something the person will actually keep. CRITICAL - COMPLETENESS OVER SIZE: keep the tool focused and compact so it comes back COMPLETE and fast - one strong tool that fully works beats a sprawling one that gets cut off. Close every function and bracket. Return complete runnable code top to bottom." },
...
.wordmark{font:600 20px/1 'Playfair Display';white-space:nowrap;color:var(--ink);display:inline-flex;align-items:center;letter-spacing:.01em;}
...
.nerves-label{font:italic 400 11px/1 'Playfair Display';color:var(--ink3);display:block;text-align:center;margin-bottom:5px;letter-spacing:.02em;white-space:nowrap;}
...
.imed-title{font:600 13px 'Inter';color:var(--ink);}
.imed-x{background:none;border:none;color:var(--ink3);font-size:15px;cursor:pointer;padding:2px 6px;}
.imed-load{padding:30px;text-align:center;color:var(--ink3);font:400 12px 'Inter';}
...
.imed-b{background:var(--bg);border:1px solid var(--stone);border-radius:8px;padding:8px 11px;font:600 11.5px 'Inter';color:var(--ink2);cursor:pointer;}
.imed-b:hover{border-color:var(--gold);color:var(--gold);}
.imed-b.on{background:var(--gold);color:#fff;border-color:var(--gold);}
.imed-sl{display:flex;align-items:center;gap:10px;font:500 11px 'Inter';color:var(--ink3);}
...
.imed-in{width:100%;background:var(--bg);border:1px solid var(--stone);border-radius:8px;padding:9px 11px;font:400 12.5px 'Inter';color:var(--ink);}
.imed-in::placeholder{color:var(--ink3);}
.imed-save{margin-top:2px;background:var(--gold);color:#fff;border:none;border-radius:9px;padding:11px;font:700 13px 'Inter';cursor:pointer;}
.imed-save:disabled{opacity:.5;cursor:default;}
.imed-editbtn{margin-top:9px;background:var(--bg);border:1px solid var(--gold);color:var(--gold);border-radius:9px;padding:9px 14px;font:600 12.5px 'Inter';cursor:pointer;display:inline-flex;align-items:center;gap:6px;}
...
.rw-title{font:600 13px 'Inter';color:var(--ink);}
.rw-count{font:600 11px 'Inter';color:var(--gold);}
.rw-start{width:100%;background:var(--gold);color:#fff;border:none;padding:13px;font:700 13px 'Inter';cursor:pointer;}
.rw-stage{width:100%;height:260px;background:#0e0d0c;}
.rw-stage canvas{display:block;width:100%;height:100%;}
.rw-fallback{padding:16px;font:400 12.5px/1.5 'Inter';color:var(--ink3);}
.rw-step{padding:13px 14px 15px;}
.rw-partname{font:700 13.5px 'Inter';color:var(--gold);margin-bottom:5px;}
.rw-inst{font:400 13px/1.55 'Inter';color:var(--ink);margin-bottom:12px;}
.rw-ctrls{display:flex;gap:7px;align-items:center;}
.rw-b{flex:1;background:var(--bg);border:1px solid var(--stone);border-radius:8px;padding:9px 6px;font:600 11.5px 'Inter';color:var(--ink2);cursor:pointer;}
...
.chats-count{position:absolute;top:-6px;right:-6px;min-width:16px;height:16px;border-radius:999px;background:var(--gold);color:#fff;font:600 9px 'Inter';display:flex;align-items:center;justify-content:center;padding:0 4px;}
...
.chats-dd-h{font:600 11px 'Inter';letter-spacing:.06em;text-transform:uppercase;color:var(--ink3);padding:8px 8px 6px;}
.chats-dd-new{width:100%;text-align:left;font:600 13px 'Inter';color:var(--gold);background:none;border:none;border-radius:9px;padding:9px 8px;cursor:pointer;}
.chats-dd-new:hover{background:var(--bg);}
.chats-dd-empty{font:400 12px/1.5 'Inter';color:var(--ink3);padding:8px;}
...
.chats-dd-title{font:500 13px 'Inter';color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;}
.chats-dd-when{font:400 10px 'Inter';color:var(--ink3);}
...
.lang-pick span{font:600 11.5px 'Inter';color:var(--ink2);}
.lang-pick select{background:var(--bg);border:1px solid var(--stone);border-radius:7px;padding:5px 8px;font:500 12px 'Inter';color:var(--ink);cursor:pointer;}
.attach-explain{background:var(--gold)!important;color:#fff!important;border:none!important;border-radius:6px!important;padding:3px 9px!important;font:600 10.5px 'Inter'!important;cursor:pointer;margin-left:auto;}
.theme-tog{background:var(--bg);border:1px solid var(--stone);border-radius:7px;padding:5px 12px;font:600 12px 'Inter';color:var(--ink);cursor:pointer;}
.offline-bar{background:var(--err);color:#fff;font:600 12px 'Inter';text-align:center;padding:8px 12px;position:sticky;top:0;z-index:5000;}
...
.sfx-in{flex:1;border:1px solid var(--stone);border-radius:9px;padding:9px 11px;font:400 13px 'Inter';color:var(--ink);background:var(--panel);outline:none;}
.sfx-in:focus{border-color:var(--gold);}
.cite{font:500 10.5px 'Inter';color:var(--ink3);background:var(--bg);border:1px solid var(--stone);border-radius:999px;padding:4px 10px;text-decoration:none;}
...
.starter{font:400 13px 'Inter';color:var(--ink2);background:var(--panel);border:1px solid var(--stone);border-radius:12px;padding:9px 14px;cursor:pointer;text-align:left;transition:all .12s;}
...
.qa-chip{background:var(--gold-s,#F4EDE0);border:1px solid var(--gold);border-radius:16px;padding:5px 13px;font:600 12px 'Inter';color:var(--gold);cursor:pointer;transition:all .12s;}
.qa-chip:hover,.qa-chip:active{background:var(--gold);color:#fff;}
.followup-chip{font:500 12.5px 'Inter';color:var(--gold);background:var(--panel);border:1px solid var(--gold);border-radius:999px;padding:8px 14px;cursor:pointer;transition:all .12s;}
.followup-chip:hover{background:var(--gold);color:#fff;}
.code-copy{position:absolute;top:8px;right:8px;z-index:2;font:600 10px 'Inter';color:var(--ink2);background:var(--bg);border:1px solid var(--stone);border-radius:7px;padding:4px 9px;cursor:pointer;}
...
.util-tx b{font:600 13.5px 'Inter';color:var(--ink);}
.util-tx span{font:400 11px 'Inter';color:var(--ink3);margin-top:2px;}
.greet-spark{margin-top:5px;font:400 12px/1.4 'Inter';color:var(--gold);opacity:.85;text-align:center;max-width:360px;margin-left:auto;margin-right:auto;}
.greet-streak{font:600 12.5px 'Inter';color:var(--gold);margin-bottom:8px;}
...
.ih-tx b{font:700 12.5px 'Inter';color:var(--ink);}
.ih-tx span{font:500 11px/1.4 'Inter';color:var(--ink2);}
...
.create-strip-div{display:flex;align-items:center;gap:12px;color:var(--ink3);font:400 11px/1 'Inter';letter-spacing:.05em;margin-bottom:11px;}
.vid-rendering{background:var(--panel);border:1px solid var(--stone);border-radius:14px;padding:16px 18px;}
.vid-rendering-top{display:flex;align-items:center;font:500 14px/1.2 'Inter';color:var(--ink);}
...
.vid-sub{font:400 12px/1.5 'Inter';color:var(--ink3);}
...
.create-cc .cc-name{font:500 11px/1 'Inter';color:var(--ink2);}
              ...
              <div className="thread">
                {thread.length===0&&!coreBusy&&(
                  ...
                  <div className="greet">
                    ...
                    {returning&&streak>1&&<div className="greet-streak">{"\uD83D\uDD25"} {streak} days in a row - good to see you.</div>}
