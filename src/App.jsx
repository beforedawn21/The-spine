...
const PIPELINES = [
  ...
  { key:"world",  name:"World",       icon:"\uD83C\uDF0D", plan:"Describe a place - Spine builds it in 3D and you stand inside it. Move your phone to look around, or switch to AR and put it in your room.", brief:"You are Spine's world engine. BUILD A PLACE THE PERSON CAN STAND INSIDE. Return ONLY JavaScript - no HTML, no markdown prose, no <script> tags. The scene is already created and running before your code is called, so never make a renderer, camera, animation loop or resize handler. THREE (r149) is loaded globally. WHAT YOU ARE GIVEN: SpineWorld.scene and SpineWorld.camera are live (camera starts at eye height y=1.6 looking down -Z, world units are METRES); SpineWorld.onFrame(function(t,dt){}) registers per-frame animation; SpineWorld.title('Name','subtitle') sets the heading; SpineWorld.say('...') speaks a line aloud and shows it as narration; SpineWorld.button('Label',function(){}) adds a tappable option to the HUD; SpineWorld.label('text',x,y,z) pins a floating label to a point in the world; SpineWorld.clearButtons() and SpineWorld.clearLabels() reset those; await SpineWorld.ask('question') asks The Spine's brain and returns text; SpineWorld.save(key,value) and SpineWorld.load(key) persist things between visits. FOUR WAYS OF BEING INSIDE IT ARE AUTOMATIC - never build controls for them: LOOK (tilt the phone to look around), WALK (hold the screen to move through the space), AR (the camera opens and your world is laid over the person's actual room), VR (split screen for a cardboard headset). Because AR exists, do NOT rely on a giant background sphere or skybox for meaning - put the meaning in real geometry so it still reads when the background is the person's living room. BUILD IT PROPERLY: real geometry, not primitives scattered about - use enough segments that curves look smooth, and compose the place out of many parts. Light it deliberately (AmbientLight for base, DirectionalLight or PointLight for shape and shadow, coloured light for mood, FogExp2 for depth). Give it atmosphere - drifting particles, gentle motion, a slow pulse - so it feels alive rather than frozen. Use MeshStandardMaterial with sensible roughness/metalness, and DoubleSide when the person will be inside a shape. Scale for a human standing there: a room is 3m tall, a tree 8m, a mountain hundreds. If the person is inside something small (a cell, an artery, a molecule), scale the world up around them instead of shrinking them. DETAIL IS THE WHOLE POINT - if someone asks to be inside a heart with a blocked artery and a stent, build the vessel wall with its texture, the plaque narrowing the channel, the stent's individual struts as a lattice, blood cells moving past, and light it from within. If they ask for a forest at night, build many trees with varied height and lean, ground, mist, moonlight, drifting fireflies, stars. ACCURACY MATTERS when the subject is real - anatomy, astronomy, architecture, engineering: get proportions, structure and terminology right, and label the parts that matter with SpineWorld.label so the person knows what they are looking at. NARRATE AND OFFER: call SpineWorld.title() and one SpineWorld.say() as the person enters, describing where they are and what to notice - warm, specific, second person, under 45 words. Then add 3-5 SpineWorld.button() options that genuinely change what they see (show it before the change, play the process, cut it open, explain it simply, move to another part). Each button should modify the scene, re-label, and say a new line. Keep a reference to anything you need to change later. PERFORMANCE: this runs on a phone - stay under roughly 60k triangles, reuse geometries and materials, avoid per-frame allocation, and never load external models or textures. CRITICAL: return complete, runnable JavaScript top to bottom, every bracket closed, no placeholder comments." },
...
(function(){
  ...
  var W = window.SpineWorld = {
    ...
    setMode:function(m){
      ...
      if(m==="ar"){ W._startCam(); } else { W._stopCam(); }
      W.mode=m;
      try{
    ...
    _boot:function(){
      ...
      W.renderer=new T.WebGLRenderer({antialias:true,alpha:true,powerPreference:"high-performance"});
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                <div className="code-bar" style={{marginTop:12}}>
                  ...
                  <button className="code-go" disabled={p.key==="game"||pipeBusy||!pipeIn.trim()} onClick={runPipeline} aria-label="Make">{pipeBusy?<span className="spinner" style={{width:15,height:15}}/>:"Go"}</button>
                </div>
                ...
                {p.key==="game"&&(
                  <div className="soon-note">
                    ...
                    <span>The game engine is still being fixed - it doesn't run reliably on every phone yet, so it's paused rather than left broken. Everything else in Create is live, and <b>World</b> builds real 3D you can stand inside today.</span>
                  </div>
                )}
                <button className="primary-cta" style={{marginTop:12}} disabled={p.key==="game"||pipeBusy||!pipeIn.trim()} onClick={runPipeline}>{pipeBusy?<><span className="spinner"/>&nbsp; The spine is composing...</>:p.key==="web"?"Build my site":p.key==="game"?"Build my game":`Make my ${p.name.toLowerCase()}`}</button>
                {p.key==="game"&&gameClarify&&(
                  <div className="clarify">
                    ...
                    <div className="clarify-actions">
                      <button className="clarify-go" onClick={runPipeline} disabled>{pipeBusy?"Building...":"Build my game"}</button>
