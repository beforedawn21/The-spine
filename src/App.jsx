...
.spine-lbl{position:absolute;top:-9px;left:14px;background:var(--gold);color:#fff;font:700 9px/1 'Manrope';letter-spacing:.12em;text-transform:uppercase;padding:3px 8px;border-radius:4px;}
...
.tools-drawer-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px 8px;}
...
export default function App(){
  ...
  async function runPipeline(){
    ...
    if(p.key==="world"&&res.live&&res.text){
      try{
        ...
        if(!err) setTimeout(()=>setPlayingGame({title:(pipeIn.trim().slice(0,40)||"World"),doc:wdoc}),120);
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    if(isCanvas){
      ...
      const cErr='(function(){var q=[];function send(m){try{parent.postMessage({__spineGame:true,crash:true,message:String(m).slice(0,300)},"*");}catch(_){}}'
        +'window.addEventListener("error",function(e){var m=String(e&&e.message||e);q.push(m);send(m);},true);'
    ...
    const __doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      +'window.addEventListener("error",function(e){__report("error",(e&&e.message)||"unknown error");},true);'
  ...
  function buildWorldDoc(rawText,title){
    ...
    const API="https://world.openfoodfacts.org https://api.open-meteo.com https://geocoding-api.open-meteo.com https://air-quality-api.open-meteo.com https://nominatim.openstreetmap.org https://restcountries.com https://api.sunrise-sunset.org";
  ...
  function previewWeb(){
    ...
    if(pipeOut&&pipeOut.key==="game"){
      ...
      setTimeout(()=>setPlayingGame({title:(pipeIn.trim().slice(0,40)||"World"),doc}),60);
      ...
      setTimeout(()=>setPlayingGame({title:(pipeIn.trim().slice(0,40)||"Tool"),doc}),60);
    }else{
  ...
  useEffect(()=>{
    function onMsg(e){
      ...
      if(d.crash){ autoFixGame((d.kind==="promise"?"Unhandled error: ":"Runtime error: ")+d.message); return; }
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {(pipeOut.key==="web"||pipeOut.key==="game"||pipeOut.key==="tool"||pipeOut.key==="world")&&pipeOut.live&&<button className="run-btn ghost" style={{marginTop:10}} onClick={previewWeb}>{"\u25b6"} {pipeOut.key==="game"?"Play game":pipeOut.key==="tool"?"Run this tool":pipeOut.key==="world"?"Enter the world":"Run it live (preview)"}</button>}
                  {pipeOut.preview&&<iframe title="preview" className="web-preview" style={pipeOut.key==="game"||pipeOut.key==="tool"?{height:"78vh",maxHeight:"680px"}:undefined} sandbox="allow-scripts allow-pointer-lock allow-forms allow-modals allow-downloads" allow="camera; microphone; geolocation; accelerometer; gyroscope; magnetometer; clipboard-write; autoplay; fullscreen; xr-spatial-tracking" srcDoc={pipeOut.preview}/>}
                  {(pipeOut.key==="game"||pipeOut.key==="tool"||pipeOut.key==="world")&&pipeOut.preview&&<button className="run-btn ghost" style={{marginTop:8}} onClick={()=>setPlayingGame({title:pipeOut.name||(pipeOut.key==="world"?"World":pipeOut.key==="tool"?"Tool":"Game"),doc:pipeOut.preview})}>{"\u26f6"} {pipeOut.key==="world"?"Enter the world":"Enlarge / full screen"}</button>}
