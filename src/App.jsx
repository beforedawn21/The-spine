...
(function(){
  ...
  var W = window.SpineWorld = {
    ...
    _boot:function(){
      ...
      W.renderer=new T.WebGLRenderer({antialias:true,alpha:true,powerPreference:"high-performance",stencil:false});
      W._pr=Math.min(window.devicePixelRatio||1,3);
      ...
      try{ W.renderer.shadowMap.enabled=true; W.renderer.shadowMap.type=T.PCFSoftShadowMap; }catch(e){}
      ...
      function loop(){
        ...
        if(W._fc>=75){
          ...
          else if(avg<0.015&&W._pr<(Math.min(window.devicePixelRatio||1,3))){ W._pr=Math.min(W._pr*1.12,Math.min(window.devicePixelRatio||1,3)); W.renderer.setPixelRatio(W._pr); }
        }
...
(function(){
  ...
  var i=0, entered=false;
  function next(){
    if(i>=CDNS.length){ document.getElementById("__esub").textContent="The 3D engine could not load. Check your connection and try again."; return; }
  ...
  function ready(){
    ...
    g.onclick=function(){
      ...
      document.getElementById("__enter").style.display="none";
      document.getElementById("__hud").style.display="block";
      try{ window.SpineWorld._boot(); }catch(e){ }
      try{ __WORLD__(); }catch(e){
      ...
      for(var k=0;k<ms.length;k++){
        (function(b){ b.onclick=function(ev){ ev.stopPropagation(); window.SpineWorld.setMode(b.getAttribute("data-m")); }; })(ms[k]);
      }
    };
  }
  next();
    ...
    (async()=>{
      try{
        if(!window.THREE){
          ...
          let loaded=false;
          for(const u of srcs){
            try{ await new Promise((res,rej)=>{const s=document.createElement("script");s.src=u;s.onload=res;s.onerror=rej;document.head.appendChild(s);}); if(window.THREE){loaded=true;break;} }catch(e){}
          }
...
.greet-streak{font:600 12.5px 'Manrope';color:var(--gold);margin-bottom:8px;}
