...
const PIPELINES = [
  ...
  { key:"tool",   name:"Tool",        icon:"🛠", plan:"Describe a real working mini-app - a calculator, tracker, scanner, form - Spine builds it, running right here.", brief:"You are Spine's tool engine. BUILD a real, working single-file mini-app (a utility, calculator, tracker, form, quiz, converter, timer, scanner, logger - whatever the user describes). Return ONE complete HTML/JS block that runs immediately in a sandboxed iframe on a phone. REQUIREMENTS: (1) clean, modern, touch-friendly UI that fills the screen, big tap targets, readable fonts, a warm off-white or dark theme; (2) fully functional - real logic, real calculations, real state, no placeholders or TODOs; (3) if it's useful, PERSIST data with a simple in-memory JS object during the session (do NOT use localStorage - it is blocked in the sandbox); (4) works OFFLINE - no external scripts, fonts, or network calls. THE PHONE'S HARDWARE IS AVAILABLE when the tool genuinely needs it - use these ONLY when relevant, and always request permission gracefully and keep the tool useful if permission is denied: CAMERA via navigator.mediaDevices.getUserMedia({video:true}) into a <video>/<canvas> (scanners, color pickers, document capture); LOCATION via navigator.geolocation.getCurrentPosition (distance, parking, local info); MOTION/ORIENTATION via DeviceMotionEvent/DeviceOrientationEvent, requesting permission with DeviceOrientationEvent.requestPermission?.() on iOS (levels, compasses, step counters, dice); HAPTICS via navigator.vibrate (timers, metronomes, feedback); VOICE OUT via speechSynthesis and VOICE IN via webkitSpeechRecognition (hands-free tools); SCREEN WAKE-LOCK via navigator.wakeLock.request('screen') (recipe/workout/teleprompter tools that must stay on); SHARE via navigator.share and clipboard via navigator.clipboard.writeText for output. Detect support before using each (if('vibrate' in navigator)...) and degrade gracefully. Make it genuinely useful and delightful, something the person will actually keep. Return complete runnable code top to bottom, closing every function and bracket." },
...
async function callClaude(system,user,model,maxTokens){
  ...
  async function __attempt(){
    // Client abort matches the server's 60s Hobby cap exactly (was 58s - gave up 2s early).
    const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),big?60000:28000);
    try{
...
.starters{display:flex;flex-direction:column;gap:8px;max-width:360px;margin:18px auto 0;}
.starter{font:400 13px 'Inter';color:var(--ink2);background:var(--panel);border:1px solid var(--stone);border-radius:12px;padding:11px 14px;cursor:pointer;text-align:left;transition:all .12s;}
.starter:hover{border-color:var(--gold);color:var(--ink);}
.starter-tool{background:linear-gradient(100deg,var(--gold-s,#F4EDE0),var(--panel));border-color:var(--gold);color:var(--gold);font-weight:600;}
.starter-tool:hover{color:var(--gold);border-color:var(--gold);}
.qa-chips{display:flex;gap:7px;flex-wrap:wrap;justify-content:center;max-width:360px;margin:16px auto 0;}
...
export default function App(){
  ...
  async function runPipeline(){
    ...
    else if(p.key==="tool"||p.key==="web") maxTok=TOK_GAME; // full mini-apps/sites need room to finish
                ...
                {thread.length===0&&!coreBusy&&(
                  ...
                  <div className="greet">
                    ...
                    <div className="starters">
                      ...
                      <button className="starter starter-tool" onClick={()=>{setTab("code");setPipe("tool");setPipeOut(null);}}>{"\uD83D\uDEE0"} Build me a tool (calculator, tracker, scanner)</button>
                    </div>
                  </div>
                  </>
                )}
