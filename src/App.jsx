...
export default function App(){
  ...
  async function doCore(override){
    ...
    const clockOnly=/^\s*(what(\x27s| is)?|whats|tell me|do you know)?\s*(the\s+)?(time|date|day|year|month)(\s+(is it|today|now|right now|is it today|is it now))?\s*(and\s+(the\s+)?(time|date|day|year|month)(\s+(is it|today|now))?)?\s*[?.!]*\s*$/i.test(q)
      ||/^\s*what (time|day|date|year|month) is it( (today|now|right now))?\s*[?.!]*\s*$/i.test(q);
  ...
  async function tryLookup(q){
    ...
    const needsWeb=(p)=>/\b(latest|current|today|tonight|right now|this (week|month|year)|recent|newest|as of|up to date|currently|202[4-9]|nowadays|frame data|stats|statistics|price|cost|score|standings|ranking|release date|weather|temperature|forecast|who (is|won|plays|owns)|when (did|was)|how many|how much|net worth|population of|capital of|ceo of|founded|located|schedule|lineup|roster|box office|record for|news|happening|open now|opening hours|in stock|available)\b/i.test(p)
      && !/^\s*(what(\x27s| is)?|whats)?\s*(the\s+)?(time|date|day|year|month)(\s+(is it|today|now|right now))?\s*[?.!]*\s*$/i.test(p);
