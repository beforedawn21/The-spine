...
.smark-i{display:inline-block;width:2.5px;height:20px;background:var(--gold-rich);border-radius:1px;position:relative;margin:0 4px;}
.smark-i::before,.smark-i::after{content:'';position:absolute;left:-3.5px;width:9px;height:1.5px;background:var(--gold-rich);}
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {(pipeOut.key==="game"||pipeOut.key==="tool")&&pipeOut.preview&&<button className="run-btn ghost" style={{marginTop:8}} onClick={()=>setPlayingGame({title:pipeOut.name||(pipeOut.key==="tool"?"Tool":"Game"),doc:pipeOut.preview})}>{"\u26f6"} Enlarge / full screen</button>}
