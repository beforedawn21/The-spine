...
export default function App(){
  ...
  // Detect when a chat message is really a "create" request, and which kind.
  function detectCreateIntent(text){
    const t=" "+text.toLowerCase().trim()+" ";
    // Words that signal a picture is wanted
    const imageNouns=/(image|images|picture|pictures|pic|pics|photo|photos|drawing|draw|art|artwork|illustration|illustrate|logo|poster|painting|paint|portrait|wallpaper|graphic|sketch|render|design|icon|avatar|emoji|meme|comic|cartoon|sticker)/;
    // Action verbs that signal creation
    const makeVerbs=/(draw|sketch|paint|generate|create|make|render|design|illustrate|show me|give me|produce|build me|whip up|cook up|imagine|visualize|picture|want|need|wanna)/;
    // Strong: a make-verb AND an image-noun anywhere
    if(makeVerbs.test(t)&&imageNouns.test(t)) return {kind:"image"};
    // Also strong: "... of ..." pattern with an image noun ("a picture of a cat")
    if(/(image|picture|photo|drawing|illustration|logo|poster|painting|portrait|sketch|render|art|graphic|icon|avatar|wallpaper) of /.test(t)) return {kind:"image"};
    // Also: starts with a draw/paint/sketch verb
    if(/^\s*(draw|paint|sketch|render|illustrate|generate|create|make|design)\b/.test(text.toLowerCase())&&imageNouns.test(t)) return {kind:"image"};
    // Music
    if(/(write|make|generate|compose|create|produce)/.test(t)&&/(song|music|melody|track|beat|tune|jingle|instrumental)/.test(t)) return {kind:"music"};
    // Video
    if(/(make|generate|create|produce)/.test(t)&&/(video|animation|animate|clip|movie|film|gif)/.test(t)) return {kind:"video"};
    // Voice / dub
    if(/(clone|dub|voice|narrate|read aloud|text to speech|tts|voiceover)/.test(t)&&/(voice|audio|speech|narration)/.test(t)) return {kind:"voice"};
    return null;
  }
  const MEDIA_SOON={
  ...
  async function doCore(){
    ...
    // TEMP DEBUG — shows what detection decided + whether image is enabled
    setThread(t=>[...t,{role:"assistant",text:"(Debug) intent="+(intent?intent.kind:"none")+" · imageEnabled="+String(settings.imageEnabled),live:true,at:new Date()}]);
    if(intent){
