...
.create-cc{background:var(--panel);border:1px solid var(--stone);border-radius:10px;padding:10px 5px;display:flex;flex-direction:column;align-items:center;gap:6px;cursor:pointer;position:relative;transition:.15s;}
.create-cc:hover{border-color:var(--gold-l);transform:translateY(-1px);}
.create-cc .cc-ic{width:27px;height:27px;border-radius:7px;background:var(--gold-s);color:var(--gold);display:flex;align-items:center;justify-content:center;font-size:14px;}
.create-cc .cc-name{font:600 11px/1 'Inter';color:var(--ink);}
.create-cc .cc-dot{width:5px;height:5px;border-radius:50%;background:var(--live);position:absolute;top:7px;right:7px;box-shadow:0 0 4px var(--live);}
.create-strip-all{text-align:center;margin-top:12px;}
@media(max-width:560px){.create-strip-grid{grid-template-columns:repeat(3,1fr);}}
              ...
              {thread.length===0&&!coreBusy&&(
                <div className="create-strip">
                  ...
                  <div className="create-strip-grid">
                    <button className="create-cc" onClick={()=>setTab("code")}><span className="cc-dot"/><span className="cc-ic">{"\u2318"}</span><span className="cc-name">Code</span></button>
                    <button className="create-cc" onClick={()=>{setTab("code");setPipe("image");}}><span className="cc-dot"/><span className="cc-ic">{"\u25a6"}</span><span className="cc-name">Image</span></button>
                    <button className="create-cc" onClick={()=>{setTab("code");setPipe("video");}}><span className="cc-dot"/><span className="cc-ic">{"\u25ae"}</span><span className="cc-name">Video</span></button>
                    <button className="create-cc" onClick={()=>{setTab("code");setPipe("music");}}><span className="cc-dot"/><span className="cc-ic">{"\u266a"}</span><span className="cc-name">Music</span></button>
                    <button className="create-cc" onClick={()=>{setTab("code");setPipe("blueprint");}}><span className="cc-dot"/><span className="cc-ic">{"\u229e"}</span><span className="cc-name">Blueprint</span></button>
                  </div>
