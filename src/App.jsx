...
const CORE_SYS = "You are Spine Core. Answer clearly, accurately and concisely. Ground answers in widely-verifiable knowledge. State uncertainty explicitly rather than guessing. Never fabricate facts.";
...
const credScore=t=>{const o=outcomePct(t);return o!=null&&o>=85&&t.runs>=100;};
...
async function callClaude(system,user){
  ...
  try{
    const res=await fetch("/api/chat",{
      ...
      signal:ctrl.signal,
      body:JSON.stringify({system,user:user||"(no input)"}),
    });
    clearTimeout(timer);const data=await res.json();
    const text=(data.content||[]).map(b=>b.type==="text"?b.text:"").join("").trim();
    if(!text)throw new Error("empty");return{text,live:true};
  }catch(e){clearTimeout(timer);
    return{text:"(Preview mode \u2014 the live model isn't reachable yet. Add your API key in Vercel to switch this on. Everything else in Spine works.)",live:false};
  }
}
...
export default function App(){
  ...
  const [coreAnswer,setCoreAnswer]=useState(null);
  ...
  function deleteAccount(){ setProfile(null);setAccount(null);setAdminAuthed(false);setDeleteOpen(false);setTab("core"); }
  ...
  const filteredTools=useMemo(()=>{
    let list=tools.filter(t=>(mcat==="all"||t.type===mcat)&&(!mq.trim()||(t.name+" "+t.desc+" "+t.by).toLowerCase().includes(mq.trim().toLowerCase())));
  ...
  const ADMIN_USER="beforedawn", ADMIN_PASS="beforedawn"; // placeholder — real secure auth with the backend
  ...
  const [signalsAbsorbed,setSignalsAbsorbed]=useState(12840);
  ...
  const pushL=useCallback(e=>setLedger(l=>[{id:Date.now()+Math.random(),t:new Date(),...e},...l]),[]);
  ...
  function buyCredits(pack){
    setBalance(b=>b+pack);setBackedCredits(c=>c+pack);
    setRP(p=>p+Math.round(pack*SPLIT.pool));
    setCompute(c=>c+Math.round(pack*SPLIT.compute));
    setSpineMargin(m=>m+Math.round(pack*SPLIT.margin));
    pushL({dir:"in",title:`Bought ${pack.toLocaleString()} credits`,from:"Card payment",to:`Pool 50 · Compute 30 · Spine 20`,amt:pack});
    setMediaMsg(null);flash();
  }
  ...
  function cashOut(){
    if(!verified){setCashMsg({warn:true,text:"Verify your identity first — required before any payout."});return;}
    const amt=Math.floor(Number(cashAmount));
    if(!amt||amt<CASHOUT_MIN){setCashMsg({warn:true,text:`Minimum cash-out is ${CASHOUT_MIN.toLocaleString()} credits = ${usd(CASHOUT_MIN)}.`});return;}
    if(amt>balance){setCashMsg({warn:true,text:`You only have ${balance} credits.`});return;}
    const m=PAYOUT_METHODS.find(x=>x.key===payoutMethod);
    setBalance(b=>b-amt);setPaidOutUsd(u=>u+amt*PEG);
    pushL({dir:"out",title:`Cash out · ${m.label}`,from:"Wallet",to:"Your account",amt});
    setCashMsg({warn:false,text:`${usd(amt)} sent via ${m.label} (${m.time}). Tax notes apply for US creators.`});
    setCashAmount("");if(!nervous)setNervous(true);
  }
  ...
  async function doCore(){
    ...
    const g=guard(coreReq);
    if(g.blocked){
      setCoreAnswer({text:g.kind==="danger"
        ...
        : "The Spine protects its members. This request touches something that needs a human's judgment, so a person is reviewing it. You'll hear back — nothing is decided by a machine alone.",live:true,guarded:true});
      setSignalsAbsorbed(n=>n+1);
      return;
    }
    setSignalsAbsorbed(n=>n+1);
    setCoreAnswer(null);setShowAudit(false);setEnsembleOuts([]);setCoreBusy(true);
    ...
    for(let i=0;i<CORE_STAGES.length;i++){
      ...
      if(CORE_STAGES[i].key==="reason"){
        if(ensembleMode){
          ...
          const outs=await Promise.all(labels.map(()=>callClaude(CORE_SYS,coreReq)));
          setEnsembleOuts(outs.map((o,idx)=>({label:labels[idx],text:o.text,live:o.live})));
          answer=outs[0].text;live=outs[0].live;
        }else{const res=await callClaude(CORE_SYS,coreReq);answer=res.text;live=res.live;}
      }else await sleep(CORE_STAGES[i].ms);
      setStatus(i,"done");
    }
    setCoreAudit({request:coreReq,sources:["Permitted workspace context","Public reference knowledge","Prior verified outputs"],
    ...
    setCoreAnswer({text:answer,live});setCoreBusy(false);flash();
  }
  function createProfile(){
    ...
    setProfile({name:pName.trim(),handle,spec:pSpec,bio:pBio.trim(),createdAt:new Date()});
  }
  function siSendCode(){
    if(!siName.trim()){setSiErr("Pick a username.");return;}
    if(!/^.+@.+\..+$/.test(siContact)&&!/^[+]?[\d ()-]{7,}$/.test(siContact)){setSiErr("Enter a valid email or phone.");return;}
    setSiErr("");setSiStep("verify");
  }
  function siVerify(){
    if(siCode.replace(/\D/g,"").length<4){setSiErr("Enter the 6-digit code.");return;}
    finishAuth(siName.trim());
  }
  function siSignIn(){
    // returning user: username + password, no re-verification
    const isAdmin=siName.trim().toLowerCase()===ADMIN_USER&&siPass===ADMIN_PASS;
    if(siName.trim().toLowerCase()===ADMIN_USER&&siPass!==ADMIN_PASS){setSiErr("Wrong password.");return;}
    if(!siName.trim()){setSiErr("Enter your username.");return;}
    if(!siPass){setSiErr("Enter your password.");return;}
    finishAuth(siName.trim(),isAdmin);
  }
  function finishAuth(name,forceAdmin){
    const isAdmin=forceAdmin||name.toLowerCase()===ADMIN_USER;
    setAccount({name:name.replace(/^@/,""),contact:siContact,verified:true,admin:isAdmin});
  ...
  function publishTool(){
    ...
    const contributors=dCollab&&dCollabH.trim()
      ...
      :sole(profile.handle);
    setTools(prev=>[{id:"u"+Date.now(),name:dName.trim(),by:profile.handle,type:dType,price:Number(dPrice),
  ...
  function answerOutcome(yes){
    setTools(prev=>prev.map(x=>x.id===runTool.id?{...x,oY:x.oY+(yes?1:0),oT:x.oT+1}:x));
    setOutcomeDone(true);
  }
  ...
  const NAV=[["core","Core","spinal cord"],["code","Create","the pipeline"],["earn","Earn","nervous system"],["marketplace","Market","the exchange"],["studio","Studio","your identity"],["wallet","Wallet","nervous center"],["vault","Vault","your memory"],["overview","Spine","the whole body"]];
  ...
  return(
    <div className="spa">
      ...
      <div className="shell">
        ...
        {account&&(
          <div className="signed-as">Signed in as <b>{account.name}</b> · <span className="verified-dot">verified</span>{account.admin&&<> · <button className="bs-open-link" onClick={()=>setAdminOpen(true)}>Brain Stem</button></>} · <button className="acct-link" onClick={()=>{setAccount(null);setAdminAuthed(false);}}>Sign out</button> · <button className="acct-link danger" onClick={()=>setDeleteOpen(true)}>Delete account</button></div>
        )}
        ...
        {tab==="overview"&&(
          <div className="g2">
            ...
            <div className="panel">
              ...
              <p className="sub">Whether you've hit a problem, want to partner, or want to invest — reach the team behind The Spine directly. Email <a href="mailto:support@thespine.cloud" className="gold-link">support@thespine.cloud</a> or use the form.</p>
          ...
          <div className="g2">
            ...
            <div className="panel">
              ...
              {codeOut&&(<>
                ...
                {codeOut.live?<div className="run-live"><span className="dot"/>Reasoned live through the spinal cord</div>:<div className="run-live off"><span className="dot"/>Preview — live model unavailable here</div>}
            ...
            {pipe!=="code"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
        ...
        {tab==="core"&&(
          <div className="g2">
            ...
            <div className="panel">
              <span className="eyebrow">Send a signal</span>
              <h2 className="sh serif">One answer — from the most sufficient process, not the cheapest shortcut</h2>
              <div style={{display:"flex",alignItems:"center",gap:10,marginTop:14,flexWrap:"wrap"}}>
                <label style={{display:"flex",alignItems:"center",gap:7,cursor:"pointer",fontSize:13,color:"var(--ink2)"}}>
                  <input type="checkbox" checked={ensembleMode} onChange={e=>setEnsembleMode(e.target.checked)} style={{accentColor:"var(--gold)"}}/>
                  High-stakes mode — ask 3 models and compare
                </label>
              </div>
              <div className="field" style={{marginTop:12}}><label>Your signal</label>
                <textarea value={coreReq} onChange={e=>setCoreReq(e.target.value)} placeholder="Ask anything — the spinal cord routes it the right way and shows its path." rows={3}/>
              </div>
              <button className="primary-cta" disabled={coreBusy||!coreReq.trim()} onClick={doCore}>
                {coreBusy?<><span className="spinner"/>&nbsp; Traveling the spinal cord…</>:"Send signal up the spinal cord"}
              </button>
              {coreStages.some(s=>s.status!=="idle")&&(
              ...
              {coreAnswer&&!ensembleMode&&(
                <>
                  <div className="answer-card">
                    <div className="ach">Answer · from the spinal cord</div>
                    <div className="act">{coreAnswer.text}</div>
                    {coreAnswer.live?<div className="run-live"><span className="dot"/>Reasoned live, then verified</div>:<div className="run-live off"><span className="dot"/>Preview — live model unavailable here</div>}
                  </div>
        ...
        <footer className="sp-foot">
          ...
          <div className="foot-legal">© {new Date().getFullYear()} Beforedawn · The Spine · <a href="mailto:support@thespine.cloud">support@thespine.cloud</a> · AI can make mistakes — verify anything important. Not legal, financial, or medical advice.
            ...
            <div className="modal-body">
              ...
              {siMode==="signin"?(
                ...
                <>
                  ...
                  <div className="field"><label>Email or phone (we verify, to keep it trusted)</label>
                    <input value={siContact} onChange={e=>{setSiContact(e.target.value);setSiErr("");}} placeholder="you@email.com  or  +1 555 000 1234" onKeyDown={e=>e.key==="Enter"&&siSendCode()}/>
                  </div>
                  {siErr&&<div className="snotice warn">{siErr}</div>}
                  <button className="primary-cta" onClick={siSendCode}>Send my code</button>
                  <div className="snotice info" style={{marginTop:10}}>Fast and trusted: verifying email or phone keeps fake accounts and fraud out. (Demo — code is any 6 digits.)</div>
                </>
              ):(
                <>
                  <p className="sub">We sent a 6-digit code to <b>{siContact}</b>. Enter it to finish.</p>
                  <div className="field"><label>Verification code</label>
                    <input value={siCode} onChange={e=>{setSiCode(e.target.value.replace(/\D/g,"").slice(0,6));setSiErr("");}} placeholder="••••••" inputMode="numeric" onKeyDown={e=>e.key==="Enter"&&siVerify()}/>
                  </div>
                  {siErr&&<div className="snotice warn">{siErr}</div>}
                  <button className="primary-cta" onClick={siVerify}>Verify & enter</button>
                  <button className="run-btn ghost" style={{marginTop:8}} onClick={()=>setSiStep("start")}>← Back</button>
                </>
              )}
            </div>
          </div>
      ...
      {adminOpen&&(
        <div className="modal-back" onClick={()=>setAdminOpen(false)}>
          <div className="modal bs-modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-h">
              ...
              {!adminAuthed?(
                <div>
                  ...
                  <div className="field"><label>Admin password</label>
                    <input type="password" value={adminPass} onChange={e=>{setAdminPass(e.target.value);setAdminErr(false);}} onKeyDown={e=>e.key==="Enter"&&(adminUser===ADMIN_USER&&adminPass===ADMIN_PASS?setAdminAuthed(true):setAdminErr(true))} placeholder="password"/>
                  </div>
                  {adminErr&&<div className="snotice warn">Not recognized. Try again.</div>}
                  <button className="primary-cta" onClick={()=>adminUser===ADMIN_USER&&adminPass===ADMIN_PASS?setAdminAuthed(true):setAdminErr(true)}>Enter the Brain Stem</button>
                  <div className="snotice info" style={{marginTop:12}}>Placeholder admin credentials for the prototype. Real, secure authentication arrives with the backend.</div>
                </div>
              ):(
        ...
        <div className="modal-back" onClick={closeRun}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {runOut&&(<>
                <div className={`run-out${runTool.type==="code"?" code":""}`}>{runOut.text}</div>
                ...
                {!runOut.error&&!outcomeDone&&(
                  <div className="outcome-ask">
                    ...
                    <div className="outcome-btns">
                      <button onClick={()=>answerOutcome(true)}>Yes, it worked</button>
                      <button onClick={()=>answerOutcome(false)}>Not quite</button>
                    </div>
                  </div>
                )}
