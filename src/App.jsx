...
// Status of each Create pipe, shown as a small bulb next to the icon:
// "green" = live and working, "yellow" = partial/being finished, "red" = not live yet.
// This is display-only signalling for the Create section; it changes no behavior.
const PIPE_STATUS = {
  image:"green", text:"green", web:"green", blueprint:"green",
  game:"yellow",
  video:"red", music:"red", movie:"red", dub:"red", agent:"red",
  community:"red", data:"red", compute:"red", prompt:"red",
};
const PIPE_STATUS_COLOR = { green:"#2ecc71", yellow:"#f1c40f", red:"#e74c3c" };
...
async function callClaude(system,user,model,maxTokens){
  ...
  const ctrl=new AbortController();const timer=setTimeout(()=>ctrl.abort(),big?58000:28000);
  try{
    ...
    return{text:"Sorry - couldn't reach the reasoning center ("+why+"). Please try again in a moment.",live:false};
  }
}
...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      // T1: fire DOMContentLoaded + load at SEVERAL delays (not once), so a game that registers its
      // startup handler slightly late still gets a fire. Also re-invoke any captured window.onload.
      +'try{if(document.readyState!=="loading"){[0,120,350,700].forEach(function(__d){setTimeout(function(){try{document.dispatchEvent(new Event("DOMContentLoaded"));}catch(e){}try{window.dispatchEvent(new Event("load"));}catch(e){}try{if(typeof __capturedOnload==="function")__capturedOnload();}catch(e){}},__d);});}}catch(e){}'
              ...
              <div className="pipe-switch" style={{marginTop:14}}>
                ...
                {PIPELINES.map(p=>(
                  <button key={p.key} className="pipe-tab" aria-pressed={pipe===p.key||undefined} onClick={()=>{setPipe(p.key);setPipeOut(null);setPipeUpload(null);setGameClarify(null);setGamePost(null);}}>
                    <span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",marginRight:6,background:PIPE_STATUS_COLOR[(settings.pipeStatus&&settings.pipeStatus[p.key])||PIPE_STATUS[p.key]]||"#999",boxShadow:"0 0 5px "+(PIPE_STATUS_COLOR[(settings.pipeStatus&&settings.pipeStatus[p.key])||PIPE_STATUS[p.key]]||"#999"),verticalAlign:"middle"}} title={((settings.pipeStatus&&settings.pipeStatus[p.key])||PIPE_STATUS[p.key])==="green"?"Working":((settings.pipeStatus&&settings.pipeStatus[p.key])||PIPE_STATUS[p.key])==="yellow"?"Partial":"Coming soon"}></span>
                ...
                <div className="bs-grid">
                  ...
                  <div className="bs-section bs-controls-section">
                    ...
                    <div className="bs-controls">
                      ...
                      <div className="bs-ctrl wide" style={{borderTop:"1px solid var(--stone)",paddingTop:10,marginTop:4}}><span style={{fontWeight:600,color:"var(--ink2)"}}>Create pipe status lights</span></div>
                      {PIPELINES.map(p=>{const cur=(settingsDraft.pipeStatus&&settingsDraft.pipeStatus[p.key])||PIPE_STATUS[p.key]||"red";return(
                      <label key={"ps-"+p.key} className="bs-ctrl toggle"><span><span style={{display:"inline-block",width:8,height:8,borderRadius:"50%",marginRight:6,background:PIPE_STATUS_COLOR[cur]||"#999",verticalAlign:"middle"}}></span>{p.name}</span>
                        <select value={cur} onChange={e=>setSettingsDraft(d=>({...d,pipeStatus:{...(d.pipeStatus||{}),[p.key]:e.target.value}}))}>
                          <option value="green">Working (green)</option>
                          <option value="yellow">Partial (yellow)</option>
                          <option value="red">Coming soon (red)</option>
                        </select></label>
                      );})}
