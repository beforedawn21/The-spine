          ...
          <div className="g2">
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {p.key==="tool"&&<div className="hint" style={{marginTop:8}}>The Spine builds a real, working mini-app that runs right here. When it helps, your tool can use the phone's camera, location, motion sensors, vibration, voice, and screen wake-lock - all with your permission. Try "a bubble level", "a barcode scanner", "a hands-free recipe timer", or "a tip calculator".</div>}
                {p.key==="tool"&&(
                  <div className="game-ex" style={{marginTop:8}}>
                    <span className="game-ex-label">Tap an idea, or write your own:</span>
                    {["A bubble level (uses motion sensors)","A guitar tuner (uses the mic)","A barcode scanner (uses the camera)","A hands-free recipe timer (talks to you)","A color picker from the camera","A compass + step counter","A tip & bill splitter","A habit tracker with streaks"].map((ex,i)=>(
                      <button key={i} type="button" className="game-ex-chip" onClick={()=>setPipeIn(ex)}>{ex}</button>
                    ))}
                  </div>
                )}
