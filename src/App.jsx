...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    if(isCanvas){
      ...
      // strip document scaffolding if the model returned a full doc without a <body> match
      cInner=cInner.replace(/<style[\s\S]*?<\/style>/gi,"").replace(/<!doctype[^>]*>/gi,"").replace(/<\/?html[^>]*>/gi,"").replace(/<head[\s\S]*?<\/head>/gi,"").replace(/<\/?body[^>]*>/gi,"");
      // escape any literal </script> in the game so it can't close our injection early
      cInner=cInner.replace(/<\/script>/gi,"<\\/script>");
      const cErr='window.addEventListener("error",function(e){try{parent.postMessage({__spineGame:true,crash:true,message:String(e&&e.message||e)},"*");}catch(_){}},true);window.addEventListener("unhandledrejection",function(e){try{parent.postMessage({__spineGame:true,crash:true,message:String(e&&e.reason&&e.reason.message||e.reason||"rejection")},"*");}catch(_){}});';
      return '<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover">'
        +'<meta http-equiv="Content-Security-Policy" content="default-src \'none\'; script-src \'unsafe-inline\'; style-src \'unsafe-inline\'; img-src \'self\' data: blob:; media-src \'self\' data: blob:; font-src \'self\' data:; connect-src \'none\'; base-uri \'none\'; form-action \'none\'; object-src \'none\'">'
        +'<style>html,body{margin:0;padding:0;width:100%;height:100%;overflow:hidden;background:#10131a;}canvas{display:block;}'+cStyle+'</style>'
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                {pipeOut&&(<>
                  ...
                  {pipeOut.key==="game"&&pipeOut.preview&&<button className="run-btn ghost" style={{marginTop:8}} onClick={()=>setPlayingGame({title:pipeOut.name||"Game",doc:pipeOut.preview})}>{"\u26f6"} Enlarge / full screen</button>}
                ...
                {thread.length===0&&!coreBusy&&(
                  ...
                  <div className="greet">
                    ...
                    <div className="starters">
                      {["Explain a complex topic simply","Draft something I need to write","Make me an image of an idea"].map((s,i)=>(
