...
export default function App(){
  ...
  async function runPipeline(){
    ...
    if(p.key==="world"&&res.live&&res.text){
      try{
        const wdoc=buildWorldDoc(res.text,(pipeIn.trim().slice(0,60)||"Your world"));
        setPipeOut(o=>({...o,preview:wdoc}));
        setTimeout(()=>setPlayingGame({title:(pipeIn.trim().slice(0,40)||"World"),doc:wdoc}),120);
      }catch(e){}
    }
  ...
  function buildWorldDoc(rawText,title){
    let code=rawText||"";
    const fence=code.match(/```(?:javascript|js|html)?\s*\n?([\s\S]*?)```/);
    if(fence) code=fence[1];
    // if it came back as a page, take the script out of it
    if(/<script[\s>]/i.test(code)){
      let js="";
      code.replace(/<script(?![^>]*src=)[^>]*>([\s\S]*?)<\/script>/gi,(m,inner)=>{js+="\n"+inner;return "";});
      if(js.trim()) code=js;
    }
    code=code.replace(/<[^>]+>/g,m=>/^<\/?(script|html|head|body|meta|link|style)/i.test(m)?"":m);
