...
WHAT YOU CAN CREATE: The Spine CAN generate real images, music, voices, videos, games, websites, and blueprints - all live, all real. If someone asks for a picture, the system tries to create it automatically; if it doesn't render inline, tell them exactly where to go: "Tap the Create tab, then choose Image." Never say "I can't make images/music/video" - they are all live. You can also SEE images and READ documents (PDFs) people share with you, READ any web link they paste, and hear them through the mic. You can write code, plans, scripts, and full text of almost anything.
...
export default function App(){
  ...
  async function doGenerateVideo(){
    ...
    try{
      ...
      if(data.url){ stopTick(); setVidOut({url:data.url}); addProjectAsset("video",{url:data.url,from:"video"}); }
      else if(data.taskId){
        ...
        for(let i=0;i<48&&!done;i++){ // ~4 min max (48 x 5s)
          ...
          if(pd.status==="success"&&pd.url){ stopTick(); setVidOut({url:pd.url}); addProjectAsset("video",{url:pd.url,from:"video"}); done=true; }
          ...
          <div className="g2">
            <div className="panel" style={{gridColumn:"1 / -1"}}>
              ...
              <div style={{display:"flex",gap:8,marginTop:6,flexWrap:"wrap",marginBottom:14}}>
                ...
                {settings.showEarn!==false&&<button className="run-btn ghost" style={{width:"auto",padding:"0 16px",marginTop:0}} onClick={()=>setTab("earn")}>Earn</button>}
              </div>
              {projects.length===0?(
