...
const LAYERS = [
  { k:"Identity",     icon:"o", d:"Who is doing the work. Owned by the person, portable everywhere.",               tab:"studio" },
  { k:"Contribution", icon:"*", d:"What was created, improved, or shipped - measured against real outcomes.",        tab:"earn" },
  { k:"Attribution",  icon:"to", d:"How value is traced across collaborators and time.",                              tab:"marketplace" },
  { k:"Settlement",   icon:"o", d:"How money is paid when value is verified - by agreed rules, never discretion.",   tab:"wallet" },
];
const SPINE_SEGS = [
...
const PIPELINES = [
  ...
  { key:"dub",    name:"Dub / clone", icon:"o", plan:"Plan dubbing or a voice - language, tone, timing, consent.", brief:"You are Spine's dubbing director. Produce a dubbing plan: target language, voice tone and pacing, a timed line sheet, and an explicit consent/likeness checklist that must be satisfied before any voice is cloned." },
...
const TOOL_TYPES = [
  ...
  { key:"code",   label:"Code / Function",  icon:"Cmd", blurb:"JavaScript sandboxed in the buyer's browser." },
  { key:"api",    label:"API Workflow",     icon:"swap", blurb:"Define a workflow - the spine maps and returns results." },
  { key:"media",  label:"Image / Video",    icon:"#", blurb:"Direct a scene - the spine renders it." },
  { key:"game",   label:"Game / Experience",icon:"o", blurb:"Design it - the spine generates a playable scene." },
          ...
          <div className="g2">
            ...
            <div className="panel">
              ...
              <div className="ov-pipe-row">
                <button className="ov-pipe" onClick={()=>setTab("core")}><span className="oi">o</span>Ask anything</button>
                <button className="ov-pipe" onClick={()=>{setPipe("code");setTab("code");}}><span className="oi">Cmd</span>Code</button>
                {PIPELINES.map(p=>(
                ...
                <button className="ov-pipe" onClick={()=>setTab("marketplace")}><span className="oi">swap</span>Market</button>
                <button className="ov-pipe" onClick={()=>setTab("vault")}><span className="oi">o</span>Vault</button>
              </div>
          ...
          <div className="g2">
            <div className="panel">
              ...
              <div className="pipe-switch" style={{marginTop:14}}>
                <button className="pipe-tab" aria-pressed={pipe==="code"||undefined} onClick={()=>setPipe("code")}><span className="pi-ic">Cmd</span>Code</button>
                {PIPELINES.map(p=>(
            ...
            <div className="panel">
              ...
              <div className="code-bar">
                ...
                <button className="code-go" disabled={codeBusy||!codeIn.trim()} onClick={runCode} aria-label="Run">
                  {codeBusy?<span className="spinner" style={{width:15,height:15}}/>:"to"}
                </button>
              </div>
              {codeMode==="debug"&&(
              ...
              {codeOut&&(<>
                ...
                {(codeMode==="generate"||codeMode==="debug"||codeMode==="refactor")&&(
                  <button className="run-btn ghost" style={{marginTop:12}} onClick={runGenerated}>play Run this code{codeLang==="javascript"||codeLang==="typescript"?" sandboxed":""}</button>
                )}
                {codeRunOut&&(<>
            ...
            {pipe!=="code"&&pipe!=="image"&&pipe!=="video"&&(()=>{const p=PIPELINES.find(x=>x.key===pipe);if(!p)return null;return(
              <div className="panel">
                ...
                <div className="code-bar" style={{marginTop:12}}>
                  ...
                  <button className="code-go" disabled={pipeBusy||!pipeIn.trim()} onClick={runPipeline} aria-label="Make">{pipeBusy?<span className="spinner" style={{width:15,height:15}}/>:"to"}</button>
                </div>
                ...
                {pipeOut&&(<>
                  ...
                  {(pipeOut.key==="web"||pipeOut.key==="game")&&pipeOut.live&&<button className="run-btn ghost" style={{marginTop:10}} onClick={previewWeb}>play Run it live (preview)</button>}
                      ...
                      {buildMode==="self"&&(<>
                        {showTemplates?(
                          ...
                          <>
                            ...
                            <div style={{marginTop:8,textAlign:"center"}}><button style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"var(--gold)"}} onClick={()=>setShowTemplates(true)}>back Browse templates</button></div>
                          </>
                        )}
                      </>)}
                    </div>
