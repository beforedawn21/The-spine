...
WHAT YOU CAN CREATE: The Spine CAN generate real images. If someone asks for a picture, drawing, or image, the system tries to create it automatically. If for any reason it doesn't generate inline, ALWAYS tell them exactly where to go: "Tap the Create tab, then choose Image - type your prompt there and I'll render it." Never flatly say "I can't make images" - image creation is live. Music and voice/dubbing are also LIVE - for those, point them to Create (Music, or Dub / clone) with delight, never say they're coming soon. Only video is still coming soon; for video, offer the text version (script, storyboard) meanwhile. You can already write code, plans, scripts, and full text of almost anything.
...
const TOK_GAME=4500;        // a full game (HTML+JS) - raised from 3000 so complex games finish (no EOF truncation)
...
const TOK_GAME_FIX=4500;    // the game auto-fix regeneration - matches TOK_GAME so the fix isn't truncated
...
export default function App(){
  ...
  function buildGameDoc(rawText,gameMode){
    ...
    const __doc='<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      // LOST-CONTEXT RECOVERY: if iOS drops the WebGL context (known Safari bug on background/foreground),
      // show a clear message instead of a silent black canvas.
      +'try{document.addEventListener("webglcontextlost",function(ev){try{ev.preventDefault();}catch(e){}__log("the game graphics were interrupted - reopen the game to continue","#fa0");},true);}catch(e){}'
